...
rem 
rem Notes: Data selectivity information for partitioned tables.
rem
...
accept p_pname prompt 'Partition Name: '
...
begin
  if '&p_pname' is not null then
     stype := 'TABLE PARTITION';
  else
     stype := 'TABLE';
  end if;
  
  sys.dbms_space.unused_space(
    upper('&p_town'), upper('&p_tname'), stype,
    tblks, tbytes, ublks, ubytes, luefid, luebid, lublk, '&p_pname'
  );
  :fblks := tblks - ublks;
end;
/
...
select :fblks blks, count(*) nrows
  from &p_town..&p_tname partition (&p_pname) ;
...
select &p_clst,
       ...
       count(*) nrows
  from &p_town..&p_tname partition (&p_pname) &p_where
group by &p_clst
order by nblks desc;
