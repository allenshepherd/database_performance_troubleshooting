...
rem Called by do.sql for Oracle v9
rem
...
select &htst_scenario_id , hseq.nextval , a.* 
  from table(dbms_xplan.display('PLAN_TABLE','&htst_stmt_id','ALL')) a;

-- If hit v9 bug 1702058, need to change to add a cast call
-- Comment previous insert and uncomment this one
--insert into  hscenario_plans 
--select &htst_scenario_id , hseq.nextval , a.* 
-- from table(cast(dbms_xplan.display('PLAN_TABLE','&htst_stmt_id','ALL') AS dbms_xplan_type_table)) a;
