Rem Copyright (c) 1995, 2014, Oracle and/or its affiliates. 
...
Rem
Rem   HISTORY
Rem     cvanes     07/09/12 -  Fix Bug#13803509 Expire cookie in Arabic
Rem     akatti     10/06/08 -  Fix bug#7433906 Add httponly support for cookie
Rem     mmuppago   03/21/06 -  Fix bug#4879467 to validate value of cookie
Rem     akatti     11/23/05 -  Fix bug#4722130 validate args of send
...
create or replace package body OWA_COOKIE is
   ...
   SC_CHAR constant varchar2(10) := chr(59); /* SemiColon */

   function validate_cookie_val(
       param in varchar2
   ) return varchar2 is
   ...
   begin
       ...
       valid_param := param;
       if instr(valid_param,(SC_CHAR)) > 0
       then
          valid_param := owa_util.validate_arg(
                         substr(valid_param,1,instr(valid_param,(SC_CHAR)) - 1));
       end if;
   ...
   begin
      -- Validate parameters
      l_name := owa_util.validate_arg(name);
      l_value := validate_cookie_val(value);
      l_path := owa_util.validate_arg(path);
      l_domain := owa_util.validate_arg(domain);
      l_secure := owa_util.validate_arg(secure);
      l_httponly := owa_util.validate_arg(httponly);
