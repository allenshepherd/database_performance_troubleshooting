Rem
Rem $Header: rdbms/admin/dbmsxpln.sql /main/27 2017/07/07 10:33:20 palashar Exp $
Rem
Rem dbmsxpln.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/dbmsxpln.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/dbmsxpln.sql
Rem SQL_PHASE: DBMSXPLN
...
Rem    palashar    06/28/17 - #(26123791): Remove i_prepare_records and
Rem                           overload prepare_records
Rem    palashar    02/27/17 - #(22273891): Add new interfaces and internal
Rem                           functions for display_cursor
Rem    kmorfoni    12/22/16 - #(25080604) add display_workload_repository and
Rem                           obsolete display_awr
Rem    kmorfoni    10/12/16 - #(24626166) add awr_location to display_awr
Rem    mzait       01/20/15 - #(20385878) longer identifier in plan_record
Rem                           for sql_profile and sql_plan_baseline
Rem    mzait       01/07/15 - #(20254870) longer identifier in plan_record
Rem    mzait       11/13/14 - #(19816978) add comments for REPORT and ADAPTIVE
Rem                           options for FORMAT argument of DISPLAY_CURSOR()
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    awaingol    05/30/12 - [proj 31793] add diag repos to prepare_records
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    awaingol    03/19/12 - [proj 31793] add get_plans_rows, get_cursor_rows,
Rem                           expose plan_table and plan_record           
Rem    mzait       09/08/11 - [Proj 33718] add con_id to display_awr
Rem    mzait       08/26/11 - [proj 31820] add ROWSTATS display format
Rem    rdongmin    08/01/11 - 28394_spm: add display_sql_profile[patch]_plan
...
create or replace package dbms_xplan AUTHID CURRENT_USER as
  ...
  ---             Further display_cursor can be used to display the plan of the
  ---             sql segment executed on the shards using the same format 
  ---             flags as coordinator plan.
  ---
  ---             Specific cursors are identified by SQL_ID and optionally a
  ---             SQL_CHILD_NUMBER. For plans from shards, format flags can be  
  ---             set to display all the plans, or shard ids of interest can be
  ---             passed to the function.
  ---
  ---             The DEFAULT without any parameters shows the last executed 
  ---             statement of the session without any plans from shards (if
  ---             the query is a shard query).
  ---
  ---             NOTE: To use the DISPLAY_CURSOR functionality, the calling
  ---             user must have SELECT privilege on V$SQL_PLAN_STATISTICS_ALL,
  ---             V$SQL, V$SQL_PLAN, and V$SQL_SHARD. By default, only the 
  ---             select_catalog role has the SELECT privilege on these views.
  ---
  ---   - DISPLAY_AWR (*** DEPRECATED) displays the execution plans for SQL
  ---             statements stored in the Automatic Workload Repository (AWR).
  ---             Important: This function is deprecated as of 12.2.0.2. Use
  ---             DISPLAY_WORKLOAD_REPOSITORY instead. DISPLAY_AWR only works
  ---             with snapshots for the local DBID, whereas
  ---             DISPLAY_WORKLOAD_REPOSITORY supports all snapshots inside AWR,
  ---             including remote and imported snapshots.
  ---             NOTE: To use the DISPLAY_AWR functionality, the calling user
  ---             must have SELECT prvilege on the following views:
  ---             DBA_HIST_SQL_PLAN, AWR_ROOT_SQL_PLAN, AWR_PDB_SQL_PLAN,
  ---             DBA_HIST_SQLTEXT, AWR_ROOT_SQLTEXT, and AWR_PDB_SQLTEXT.
  ---             By default, select privilige for these views is granted to the
  ---             select_catalog role.
  ---
  ---   - DISPLAY_WORKLOAD_REPOSITORY displays the execution plans for SQL
  ---             statements stored in the Automatic Workload Repository (AWR).
  ---             It supports all snapshots in AWR, including remote and
  ---             imported snapshots.
  ---             NOTE: To use the DISPLAY_WORKLOAD_REPOSITORY functionality,
  ---             the calling user must have SELECT prvilege on the following
  ---             views: DBA_HIST_SQL_PLAN, AWR_ROOT_SQL_PLAN, AWR_PDB_SQL_PLAN,
  ---             DBA_HIST_SQLTEXT, AWR_ROOT_SQLTEXT, and AWR_PDB_SQLTEXT.
  ---             By default, select privilige for these views is granted to the
  ---             select_catalog role.
  ---
  ...
  ---        
  ---        REPORT:  Displays a report that shows the plan the optimizer would
  ---                 have picked if automatic re-optimization was used.
  ---                 This format can be used under two scenarios:
  ---                 - when running in adaptive reporting mode only, i.e, with
  ---                   optimizer_adaptive_reporting_only=true
  ---                 - when displaying the first cursor of a query on which the
  ---                   automatic re-optimization is or will be applied in the
  ---                   subsequent executions.
  ---
  ---        ADAPTIVE: Displays additional information related to an adaptive
  ---                  execution to show the full plan including operations that
  ---                  were started and stopped (STATISTICS COLLECTOR) and which
  ---                  operations are marked as inactive, e.g., after a join has
  ---                  been resolved to use a particular join method.
  ---
  ...
  ---
  ---   ------------------------------------------------------------------------
  ---   (*** DEPRECATED) function display_awr(
  ---                        sql_id          varchar2,
  ---                        plan_hash_value integer  default null,
  ---                        db_id           integer  default null,
  ---                        format          varchar2 default 'TYPICAL',
  ---                        con_id          integer  default null,
  ---                        awr_location    varchar2 default 'AWR_ROOT')
  ---
  ---   function display_workload_repository(
  ---                        sql_id          varchar2,
  ---                        plan_hash_value integer  default null,
  ---                        format          varchar2 default 'TYPICAL',
  ---                        dbid            integer  default null,
  ---                        con_dbid        integer  default null,
  ---                        awr_location    varchar2 default 'AWR_ROOT')
  ---
  ---   - sql_id:
  ...
  ---        querying
  ---        - AWR_ROOT_SQL_PLAN, if the AWR to be accessed is in the root
  ---          container.
  ---        - AWR_PDB_SQL_PLAN, if the AWR to be accessed is in the local
  ---          container.
  ---
  ---   - plan_hash_value:
  ...
  ---        If this parameter is omitted or NULL, it will be defaulted to
  ---        sys_context('userenv', 'con_dbid')
  ---
  ---   - awr_location:
  ---        Specifies the location of the AWR repository. Supported values are:
  ---        - 'AWR_ROOT', if the AWR to be accessed is in the root container.
  ---        - 'AWR_PDB',  if the AWR to be accessed is in the local container.
  ---        The default value is 'AWR_ROOT'.
  ---
  ---   **Note**  Function DISPLAY_AWR is deprecated as of 12.2.0.2. Use
  ---             DISPLAY_WORKLOAD_REPOSITORY instead. DISPLAY_AWR only works
  ---             with snapshots for the local DBID, whereas
  ---             DISPLAY_WORKLOAD_REPOSITORY supports all snapshots inside AWR,
  ---             including remote and imported snapshots.
  ...
  ---      where s.sid=9;
  ---
  ---   3/ display the plans for the shard sql segment executed for current
  ---      or last executed statement, or for the SQL id and child number 
  ---      passed to the function. The format of coordinator plan is used for
  ---      displaying the plan for shard sql segment.
  ---      
  ---      set linesize 150
  ---      set pagesize 2000
  ---      select * 
  ...
  ---                                           'TYPICAL +ALL_SHARDS'));
  ---      
  ---      Alternatively you can execute the following after executing the
  ---      sql query as shown in 1.
  ---      
  ---      set linesize 150
  ---      set pagesize 2000
  ---      select * 
  ---      from table(dbms_xplan.display_cursor(format=>'ALL +ALL_SHARDS'));
  ---                                           
  ---      You can alse display plans for selected shards. 
  ---                                           
  ---      set linesize 150
  ---      set pagesize 2000
  ---      select * 
  ---      from table(dbms_xplan.display_cursor(format=>'BASIC +ALL_SHARDS'
  ---                                           shard_id=>11));
  ---      
  ---    - You can get shards ids using ora_shard_id and shards clause,
  ---      
  ---      select ora_shard_id from shards(dual);
  ---      
  ---      OR V$SQL_SHARD in case you are on the coordinator.
  ---      
  ---      select shard_id 
  ---      from v$sql_shards 
  ---      where sql_id = 'f98t6zufy04g5' and child_number = 0;
  ---
  ...
  -- display from AWR (deprecated; use display_workload_repository instead)
  ...
  function display_sql_profile_plan(name          varchar2,
                                    format        varchar2 default 'TYPICAL')
  return dbms_xplan_type_table
  pipelined;

  -- display from SQL patch
  function display_sql_patch_plan(name          varchar2,
                                  format        varchar2 default 'TYPICAL')
  return dbms_xplan_type_table
  pipelined;
