Rem
Rem $Header: rdbms/admin/dbmstrig.sql /main/14 2014/02/20 12:45:45 surman Exp $
Rem
Rem dbmstrig.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/dbmstrig.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/dbmstrig.sql
Rem SQL_PHASE: DBMSTRIG
...
Rem    rpang       07/23/12 - 14362485: add ora_original_sql_txt
...
/*--------------------------------------------------------------------------
  Function: ora_original_sql_txt

    Returns the SQL text of the original SQL statement triggering a server
  ...
  Parameters:
    sql_text  - SQL text (broken into multiple collection elements if
                statement is long)
  Returns:
    The number of elements in PL/SQL table

*/
create or replace function original_sql_txt (sql_text out ora_name_list_t) 
return binary_integer is
begin
return dbms_standard.original_sql_txt(sql_text);
end;
/
grant execute on original_sql_txt to public
/
create or replace public synonym ora_original_sql_txt for original_sql_txt
/
