Rem
Rem $Header: rdbms/admin/initxqry.sql /main/24 2017/04/27 17:09:44 raeburns Exp $
Rem
Rem initxqry.sql
Rem
...
Rem    SQL_SOURCE_FILE: rdbms/admin/initxqry.sql
Rem    SQL_SHIPPED_FILE: rdbms/admin/initxqry.sql
Rem    SQL_PHASE:  INITXQRY
...
Rem    yinlu       02/16/17 - bug 25547703
...
CREATE OR REPLACE PACKAGE BODY dbms_xquery AS
  ...
  FUNCTION eval(xqry varchar2) return xmltype is
    ...
    rval xmltype := null;
  begin
    execute immediate 'select xmlquery(:1 returning content) from sys.dual' 
