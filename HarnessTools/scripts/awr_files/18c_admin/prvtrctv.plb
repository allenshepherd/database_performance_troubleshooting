...
CREATE TABLE utl_recomp_skip_list(obj# number not null);
CREATE OR REPLACE VIEW utl_recomp_all_objects AS
   ...
   WHERE     o.owner# = u.user#
         ...
         AND (   o.type# IN (1, 2, 4, 5, 6, 7, 8, 9, 11, 12, 14,
                             22, 23, 24, 29, 32, 33, 42, 43, 46, 59, 62)
              OR (o.type# = 13 AND o.subname IS NULL AND
                  NOT REGEXP_LIKE(o.name, 'SYS_PLSQL_[[:alnum:]]+_[[:alnum:]]+_[12]')))
         AND (BITAND(o.flags, 128) = 0)
   UNION ALL
   SELECT s.obj#, a.ext_username owner, s.name objname, s.type#, s.namespace,
          s.status, e.name edition_name
   FROM  obj$ s, user$ a, obj$ e, edition$ ed
   WHERE     s.remoteowner IS NULL
         ...
         AND a.spare2 = e.obj#
         AND e.obj#   = ed.obj#
         AND BITAND(ed.flags, 1) <> 1;
CREATE OR REPLACE VIEW utl_recomp_invalid_all AS
    SELECT * FROM utl_recomp_all_objects
     WHERE status IN (4, 5, 6)
       AND obj# NOT IN (SELECT obj# FROM utl_recomp_compiled UNION ALL
                        SELECT obj# FROM utl_recomp_skip_list);
