Rem
Rem $Header: rdbms/admin/catfga.sql /main/43 2017/04/17 19:23:05 amunnoli Exp $
Rem
Rem catfga.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catfga.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catfga.sql
Rem SQL_PHASE: CATFGA
...
Rem    amunnoli    08/10/15 - Bug 13716158: Add current_user column to 
Rem                           dba_fga_audit_trail
Rem    akruglik    06/30/15 - Proj 47234: Get rid of scope column
Rem    hmohanku    03/03/15 - Proj:55106 Add scope to audit policy views
Rem    ratakuma    02/10/15 - Bug 20331945: use CURRENT_USER in 
Rem                           USER_AUDIT_POLICIES
Rem    nkgopal     06/24/14 - Proj 35931: Update RLS_INFO to
Rem                           DBA_COMMON_AUDIT_TRAIL
Rem    nkgopal     04/16/14 - Proj# 35931: Add rls_info to audit trail views
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    talliu      06/28/13 - Add CDB view for DBA view
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    nkgopal     08/21/11 - Bug 12794380: AUDITOR to AUDIT_VIEWER
Rem    amunnoli    02/24/11 - Grant select on dba_fga_audit_trail,
Rem                           dba_common_audit_trail to AUDIT_ADMIN
Rem                           and AUDITOR role
...
create or replace view DBA_AUDIT_POLICIES (OBJECT_SCHEMA, OBJECT_NAME,
                          ...
                          POLICY_COLUMN_OPTIONS, COMMON, INHERITED)
as
select u.name, o.name, u2.name, f.pname, f.ptxt, f.pcol,
       ...
       decode(bitand(f.stmt_type, 512), 512, 'YES', 'NO'),
       decode(bitand(f.stmt_type, 512), 
              512, decode(SYS_CONTEXT('USERENV', 'IS_APPLICATION_PDB'), 
                          'YES', 'YES', 'NO'),
              'NO')
...
/
comment on column DBA_AUDIT_POLICIES.COMMON is
'Does policy apply across multiple containers'
/
comment on column DBA_AUDIT_POLICIES.INHERITED is
'Was policy inherited from another container'
/
grant select on DBA_AUDIT_POLICIES to select_catalog_role
/

execute CDBView.create_cdbview(false,'SYS','DBA_AUDIT_POLICIES','CDB_AUDIT_POLICIES');
grant select on SYS.CDB_AUDIT_POLICIES to select_catalog_role
/
create or replace public synonym CDB_AUDIT_POLICIES for SYS.CDB_AUDIT_POLICIES
/
...
'If ALL_COLUMNS then all relevant columns apply else any of the relevant columns apply'
/
comment on column ALL_AUDIT_POLICIES.COMMON is
'Does policy apply across multiple containers'
/
comment on column ALL_AUDIT_POLICIES.INHERITED is
'Was policy inherited from another container'
/
grant select on ALL_AUDIT_POLICIES to select_catalog_role
/

execute CDBView.create_cdbview(false,'SYS','DBA_AUDIT_POLICY_COLUMNS','CDB_AUDIT_POLICY_COLUMNS');
grant select on SYS.CDB_AUDIT_POLICY_COLUMNS to select_catalog_role
/
create or replace public synonym CDB_AUDIT_POLICY_COLUMNS for SYS.CDB_AUDIT_POLICY_COLUMNS
/
...
'If ALL_COLUMNS then all relevant columns apply else any of the relevant columns apply'
/
comment on column USER_AUDIT_POLICIES.COMMON is
'Does policy apply across multiple containers'
/
comment on column USER_AUDIT_POLICIES.INHERITED is
'Was policy inherited from another container'
/
...
create or replace view DBA_FGA_AUDIT_TRAIL (
      ...
      TRANSACTIONID, STATEMENTID, ENTRYID, OBJ_EDITION_NAME, DBID, RLS_INFO,
      CURRENT_USER)
as
select 
      sessionid, 
      ...
      xid, statement, entryid, obj$edition, dbid, rls$info, current_user
from sys.fga_log$
/
...
/
comment on column DBA_FGA_AUDIT_TRAIL.RLS_INFO is
'RLS predicates along with the RLS policy names used for the object accessed'
/
comment on column DBA_FGA_AUDIT_TRAIL.CURRENT_USER is
'Effective user for the statement execution'
/
grant select on DBA_FGA_AUDIT_TRAIL to select_catalog_role
/
grant read on DBA_FGA_AUDIT_TRAIL to AUDIT_ADMIN
/
grant read on DBA_FGA_AUDIT_TRAIL to AUDIT_VIEWER
/


execute CDBView.create_cdbview(false,'SYS','DBA_FGA_AUDIT_TRAIL','CDB_FGA_AUDIT_TRAIL');
...
/
create or replace public synonym CDB_FGA_AUDIT_TRAIL for SYS.CDB_FGA_AUDIT_TRAIL
/
...
create or replace view DBA_COMMON_AUDIT_TRAIL (AUDIT_TYPE, SESSION_ID,
    ...
    LOGOFF_DLOCK, SESSION_CPU, OBJ_EDITION_NAME, DBID, RLS_INFO, CURRENT_USER)
as
...
select DECODE(BITAND(audit_type, 15), 1, 'Standard XML Audit', 
    ...
    DECODE(xad.action,
        ...
        Null),                                       /* SYS_PRIVILEGE */
    DECODE(xad.action, 
        108 /* grant  sys_priv */, SUBSTRB(auth_privileges,1,1), 
        109 /* revoke sys_priv */, SUBSTRB(auth_privileges,1,1), 
        114 /* grant  role */,     SUBSTRB(auth_privileges,1,1),
        115 /* revoke role */,     SUBSTRB(auth_privileges,1,1), 
        Null),                                        /* ADMIN_OPTION */
    OS_PRIVILEGE, GRANTEE, spx.name, SES_ACTIONS, 
    Null, Null, Null, Null, Null, Null, OBJ_EDITION_NAME, DBID,
    TO_CLOB(RLS_INFO), CURRENT_USER
...
select 'Fine Grained Audit', SESSION_ID,
    ...
    Null, Null, Null, OBJ_EDITION_NAME, DBID, RLS_INFO, CURRENT_USER
...
/
grant read on DBA_COMMON_AUDIT_TRAIL to AUDIT_ADMIN
/
grant read on DBA_COMMON_AUDIT_TRAIL to AUDIT_VIEWER
/
...
/
comment on column DBA_COMMON_AUDIT_TRAIL.RLS_INFO is
'RLS predicates along with the RLS policy names used for the object accessed'
/
comment on column DBA_COMMON_AUDIT_TRAIL.CURRENT_USER is
'Effective user for the statement execution'
/

execute CDBView.create_cdbview(false,'SYS','DBA_COMMON_AUDIT_TRAIL','CDB_COMMON_AUDIT_TRAIL');
...
/
create or replace public synonym CDB_COMMON_AUDIT_TRAIL for SYS.CDB_COMMON_AUDIT_TRAIL
/
