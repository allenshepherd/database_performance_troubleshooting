Rem
Rem $Header: rdbms/admin/dbmswrr.sql /main/76 2017/11/16 14:50:01 josmamar Exp $
Rem
Rem dbmswrr.sql
Rem
...
Rem    josmamar    11/13/17 - bug 26989795: add load_long_sqltext API 
Rem    qinwu       05/22/17 - bug 26114372: encrypt/decrypt capture
Rem    qinwu       03/20/17 - proj 70152: change start_capture for encryption
Rem    qinwu       02/15/17 - change to invokers rights
Rem    yberezin    11/23/16 - fix parameter description
Rem    yberezin    09/14/16 - fix wrong comment for reuse_replay_filter_set
Rem    qinwu       06/06/16 - bug 22986237: support sql skip or remap
Rem    josmamar    06/15/16 - bug 23131301: change process_capture API
Rem    qinwu       03/14/16 - bug 22881453: consolidated replay default SCN
Rem    yberezin    02/12/16 - increase threads_per_process for calibration
Rem    josmamar    11/03/15 - bug 21874643: change prepare_replay() API to 
Rem                           support query-only for non consolidated replay
Rem    quotran     09/15/15 - bug 21836755: new APIs to load tracked commits
Rem    quotran     08/04/15 - bug 21610276: support always-on capture
Rem    quotran     04/07/15 - bug 20827740: new APIs for RAC per-instance sync
Rem    quotran     03/05/15 - bug 20827740: change prepare_replay() API
Rem    quotran     12/03/14 - bug 20204603: change the data type of an input
Rem                           parameter of process_replay_graph()
Rem    qinwu       11/20/14 - proj 47326: change APIs for PL/SQL workload
Rem    yberezin    04/02/14 - bug 17507460
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    qinwu       09/12/13 - bug 17456719: API changes for replay info
Rem    yberezin    07/26/13 - bug 17207685
Rem    kmorfoni    06/29/13 - Bug 16993001: parallel level in
Rem                           process_replay_graph
...
Rem    kmorfoni    03/08/12 - Bug 13816019: Report for Workload Intelligence
...
CREATE OR REPLACE PACKAGE dbms_workload_capture AUTHID CURRENT_USER AS
  -- ***********************************************************
  --  START_CAPTURE
  ...
  --
  --                        The calling user must have the appropriate
  ...
  --                        
  --     plsql_mode - Capture options for PL/SQL calls:
  --             'TOP_LEVEL': top-level PL/SQL only (Default)
  --             'EXTENDED': both top-level PL/SQL and SQL executed from PL/SQL
  --     encryption - encryption algorithm: NULL (default), 'AES128', 'AES192',
  --                  or 'AES256'
  --                         
  -- ***********************************************************
  ...
  -- ************************************************************
  -- is my session being captured?
  FUNCTION get_state RETURN BINARY_INTEGER;

  -- ************************************************************
  -- START_BATCH_CAPTURE
  --   Start workload capture and store data in different buckets.
  --   For instance, workload in 9AM - 10AM will be stored in bucket 1
  --   while workload in 10AM - 12PM will be stored in bucket 2.
  --
  -- ************************************************************
  REGULAR_CAPTURE            CONSTANT   BINARY_INTEGER := 0;
  BATCH_CAPTURE              CONSTANT   BINARY_INTEGER := 1;

  PROCEDURE START_BATCH_CAPTURE(
                     name             IN VARCHAR2,
                     dir              IN VARCHAR2,
                     duration         IN NUMBER   DEFAULT NULL,
                     default_action   IN VARCHAR2 DEFAULT 'INCLUDE',
  ...
  -- ***********************************************************
  -- SWITCH_BUCKET
  --   Signals all connected sessions to store workload captures
  --   into a new bucket.
  --
  --   By default, SWITCH_BUCKET will create an AWR snapshot for
  --   the workload captured in the current bucket.
  -- ***********************************************************
  PROCEDURE switch_bucket(create_snapshot IN BOOLEAN DEFAULT TRUE);

  -- ***********************************************************
  -- ENCRYPT_CAPTURE
  --   Encrypt an un-encrypted workload capture
  --  
  -- Parameters:
  --   src_dir - a directory object pointing to the workload capture to be
  --             ecrypted (case sensitive)
  --   dst_dir - a directory object pointing to an OS path that has write
  --             permissions; the encrypted capture files will be written to
  --             this directory (case sensitive) 
  --   encryption -'AES128', 'AES192', 'AES256' (default)
  -- ***********************************************************
  PROCEDURE encrypt_capture(src_dir    IN VARCHAR2,
                            dst_dir    IN VARCHAR2,
                            encryption IN VARCHAR2 DEFAULT 'AES256');

  -- ***********************************************************
  -- DECRYPT_CAPTURE
  --   Decrypt an encrypted workload capture
  --  
  -- Parameters:
  --   src_dir - a directory object pointing to the workload capture to be
  --             decrypted (case sensitive)
  --   dst_dir - a directory object pointing to an OS path that has write
  --             permissions; the decrypted capture files will be written to
  --             this directory (case sensitive) 
  -- ***********************************************************
  PROCEDURE decrypt_capture(src_dir IN VARCHAR2, dst_dir IN VARCHAR2);
...
CREATE OR REPLACE PACKAGE dbms_workload_replay AUTHID CURRENT_USER AS
  ...
  --    Parameters:
  --
  --      capture_dir - the name of the workload capture directory object
  --          (case sensitive). The directory object must point to a valid OS
  --          directory that has appropriate permissions. New files will be
  --          added to this directory. (MANDATORY)
  --
  --      parallel_level - number of oracle processes used to process the
  --          capture in a parallel fashion. The NULL default value will
  --          auto-compute the parallelism level, whereas a value of 1 will
  --          enforce serial execution.
  --
  --      synchronization - this determines the synchronization the user will
  --                        be able to use for replay:
  --          'TIME'      : replay can use the 'TIME' synchronization only
  --          'SCN'       : (Default) replay can use 'TIME' or 'SCN'
  --          'OBJECT_ID' : replay can use 'TIME', 'SCN', or 'OBJECT_ID'
  --          See also the PREPARE_REPLAY procedure.
  --
  --      plsql_mode - the processing option for PL/SQL calls:
  --          'TOP_LEVEL' : (Default) only top-level PL/SQL will be replayed;
  --                        this will be the only option for replay
  --          'EXTENDED'  : both top-level PL/SQL and SQL executed from PL/SQL
  --                        can be replayed; replay can use either 'TOP_LEVEL'
  --                        or 'EXTENDED'
  --          See also the INITIALIZE_REPLAY procedure.
  -- ***********************************************************
  ...
  --
  --      replay_dir - name of the directory object (case sensitive) that
  --                   points to the OS directory that contains processed
  --                   capture data
  --      plsql_mode - Replay option for PL/SQL calls:
  --	              'TOP_LEVEL': (Default) replay top-level PL/SQL only
  --	              'EXTENDED' : replay the SQL executed from PL/SQL, or
  --                               top-level PL/SQL if there is no SQL recorded
  --                               inside the PL/SQL call
  --                  See also the PROCESS_CAPTURE procedure.
  --      rac_inst_list - a comma-separated list of RAC instances to use for
  --                      replay, e.g. '1,3,5'.
  --
  --    Prerequisites:
  --
  --      - Workload capture must have been already processed using
  --        DBMS_WORKLOAD_REPLAY.PROCESS_CAPTURE in the same database version.
  --      - The database state must have been logically restored to what it was
  --        at the beginning of the original workload capture.
  --
  -- ***********************************************************
  ...
  --      useful in cases where the replay is becoming noticeably slow because
  ...
  --
  --     '_AUTO_AWR_EXPORT'     - Automatically export AWR dump. Default: TRUE
  --     'DISABLE_DB_LINKS'     - Not replaying DB Links. Default: FALSE
  --     'USE_RECORDED_SYSGUID' - Use recorded value for SYS_GUID
  --                              Default: TRUE
  --     'USE_RECORDED_RANDOM'  - Use recorded value for DBMS_RANDOM
  --                              Default: TRUE
  --     'USE_RECORDED_SYSDATETIME' - Use recorded value for SYSDATE and 
  --                                  SYSTIMESTAMP. Default: TRUE
  --
  --   Prerequisites:
  --     - DBMS_WORKLOAD_REPLAY.INITIALIZE_REPLAY must have been called
  ...
  --   Reset all the advanced parameters to their default values.
  --
  --   Parameters: none
  -- ***********************************************************
  ...
  --                    is activated only when the delay is more than
  --                    min_delay. DEFAULT VALUE: 10 minutes.
  --     max_delay    - upper bound of call delay in minutes. The timeout
  --                    action throws ORA-15569 when the delay is more than
  --                    max_delay. DEFAULT VALUE: 120 minutes.
  ...
  --                    current replay elapsed time is more than the
  --                    multiplication of capture elapsed time and
  --                    delay_factor. DEFAULT VALUE: 8
  --   
  ...
  --
  --   Parameters:
  --     enabled      - TRUE if the timeout action is enabled and FALSE
  --                    otherwise.
  --     min_delay    - lower bound of call delay in minutes. The replay action
  ...
  --  PREPARE_REPLAY
  --
  --    Puts the DB state in PREPARE mode. The database should have been
  --    initialized for replay using DBMS_WORKLOAD_REPLAY.INITIALIZE_REPLAY(),
  --    and optionally any capture time connection strings that require
  ...
  --      synchronization -
  --
  --        set the synchronization mode for replay:
  --
  --        'TIME': the synchronization will be based on the time the action
  --                took place during capture.
  --
  --        'SCN':  (Default) the synchronization will be based on the
  --                capture-time commits; the capture commit order will be
  --                preserved during replay.
  --
  --        'OBJECT_ID': every replayed action will ONLY be executed AFTER the
  --                RELEVANT commits have finished execution. The relevant
  --                commits are those that were issued before the given action
  --                in the captured workload and that modified at least one of
  --                the database objects the given action is referencing
  --                (either implicitly or explicitly). This synchronization
  --                mode makes sure that any replay action will see the same
  --                data that the action saw during capture, but allows greater
  --                concurrency for the actions that do not touch the same
  --                objects/tables.
  --
  --                This value is deprecated. See also the PROCESS_CAPTURE
  --                procedure.
  --
  --                For compatibility, an overloaded version of this procedure
  --                uses BOOLEAN for this parameter:
  --                    TRUE  means 'SCN'
  --                    FALSE means 'TIME'
  --
  --      connect_time_scale -
  --
  --        Scale the time elapsed between the moment the workload capture was
  --        started and the moment a session connected to the database. The
  --        value is interpreted as percentage. Use for increasing or
  --        decreasing the number of concurrent users during replay.
  --        DEFAULT VALUE: 100
  --
  --        For example, the following was observed during capture:
  --
  --        12:00 : Capture started
  --        12:10 : First session connected  (10m after)
  --        12:30 : Second session connected (30m after)
  --        12:42 : Third session connected  (42m after)
  --
  --        If the connect_time_scale is 50, then the session connects will be
  --        replayed as follows:
  --
  --        12:00 : Replay starts
  --        12:05 : First session connects  ( 5m after)
  --        12:15 : Second session connects (15m after)
  --        12:21 : Third session connects  (21m after)
  --
  --        If the connect_time_scale is 200, then the session connects will be
  --        replayed as follows:
  --
  --        12:00 : Replay starts
  --        12:20 : First session connects  (20m after)
  --        13:00 : Second session connects (60m after)
  --        13:24 : Third session connects  (84m after)
  --
  --      think_time_scale -
  --
  --        Scale the time elapsed between two successive user calls from the
  --        same session. The value is interpreted as percentage. Use for
  --        increasing or decreasing the number of concurrent users during
  --        replay. DEFAULT VALUE: 100
  --
  --        For example, the following was observed during capture:
  --
  --        12:00 : User SCOTT connects
  --        12:10 : First user call issued (10m after the connect)
  --        12:14 : First user call completes in 4 minutes
  --        12:30 : Second user call issued (16m after completion of the
  --                previous call)
  --        12:40 : Second user call completes in 10 minutes
  --        12:42 : Third user call issued (2m after completion of the previous
  --                call)
  --        12:50 : Third user call completes in 8 minutes
  --
  --        If the think_time_scale is 50, then the user calls will be replayed
  --        as follows:
  --
  --        12:00 : User SCOTT connects
  --        12:05 : First user call issued (5m = 50% of 10m after the connect)
  --        12:10 : First user call completes in 5 minutes (takes a minute
  --                longer)
  --        12:18 : Second user call issued (8m = 50% of 16m after completion
  --                of the previous call)
  --        12:25 : Second user call completes in 7 minutes (takes 3 minutes
  --                less)
  --        12:26 : Third user call issued (1m = 50% of 2m after completion of
  --                the previous call)
  --        12:35 : Third user call completes in 9 minutes (takes a minute
  --                longer)
  --
  --      think_time_auto_correct -
  --
  --        Auto-correct the think time between calls when user calls take
  --        longer to complete during replay than they did during capture.
  --        DEFAULT VALUE: TRUE (reduce think time if replay goes more slowly
  --        than capture)
  --
  --        For example, the following was observed during capture:
  --
  ...
  --        12:50 : Third user call completes in 8 minutes
  --
  --        If think_time_scale is 100 and think_time_auto_correct is TRUE,
  --        then the user calls will be replayed as follows:
  --
  --        12:00 : User SCOTT connects
  --        12:10 : First user call issued (10m after the connect)
  --        12:15 : First user call completes in 5 minutes (takes 1 minute
  --                longer)
  --        12:30 : Second user call issued 15 minutes after completion of the
  --                previous call (16m minus the extra 1m the previous call
  --                took)
  --        12:44 : Second user call completes in 14 minutes (takes 4 minutes
  --                longer)
  --        12:44 : Third user call issued immediately (2m minus the extra 4m
  --                the previous call took)
  --        12:52 : Third user call completes in 8 minutes
  --
  --      scale_up_multiplier -
  --
  --        Define the number of times the query workload is scaled up during
  --        replay. Each captured session is replayed concurrently as many
  --        times as the value of the parameter. However, only one of the
  --        sessions in each set of identical replay sessions executes both
  --        queries and updates. The remaining sessions only execute queries.
  --
  --        More specifically:
  --
  --          - One replay session (base session) of each set of identical
  --            sessions will replay every call from capture as usual.
  --          - The remaining sessions (scale-up sessions) will only replay the
  --            read-only calls. Thus, the DDL, DML, PLSQL calls that modify
  --            the database, and SELECTs FOR UPDATE will be skipped.
  --          - The scale-up read-only calls are synchronized appropriately and
  --            obey the timings defined by think_time_scale,
  --            connect_time_scale, and think_time_auto_correct. Also, the
  --            queries wait for the appropriate commits.
  --          - No error divergence is generated for the scale-up sessions.
  --          - All base and scale-up sessions that replay the same capture
  --            file, connect from the same workload replay client process.
  --
  --      capture_sts -
  --
  --        If TRUE, a SQL Tuning Set (STS) capture is started in parallel with
  --        replay. DEFAULT VALUE: FALSE. Currently, parallel STS capture is
  --        not supported in RAC, so in RAC, the parameter is ignored.
  --
  --        The created SQL tuning set can be exported using
  --        DBMS_WORKLOAD_REPLAY.EXPORT_AWR, along with the AWR data.
  --
  --        The filters defined with the DBMS_WORKLOAD_REPLAY API do not apply
  --        to the SQL Tuning Set capture.
  --
  --        The calling user must be granted the privilege 'ADMINISTER SQL
  --        TUNING SET'.
  --
  --      sts_cap_interval -
  --
  --        Set the interval in seconds for capturing SQL from the cursor
  --        cache. DEFAULT VALUE: 300
  --
  --      rac_mode -
  --
  --        Set replay options in a RAC environment:
  --
  --          GLOBAL_SYNC:
  --            synchronization across all instances (Default)
  --          PER_INSTANCE_CLIENT:
  --            synchronization is global and each wrc client handles part of
  --            the workload that is sent to only one instance
  --          PER_INSTANCE_SYNC:
  --            synchronization is local (within each instance only) and each
  --            wrc client handles part of the workload that is sent to only
  --            one instance
  --
  --    Prerequisites:
  --
  --      - The database must have been initialized for replay using
  --        DBMS_WORKLOAD_REPLAY.INITIALIZE_REPLAY().
  --      - Any capture time connections strings that require remapping
  --        during replay must have already been remapped using
  --        DBMS_WORKLOAD_REPLAY.REMAP_CONNECTION().
  --
  -- ***********************************************************
  GLOBAL_SYNC          CONSTANT   BINARY_INTEGER := 0;
  PER_INSTANCE_CLIENT  CONSTANT   BINARY_INTEGER := 1;
  PER_INSTANCE_SYNC    CONSTANT   BINARY_INTEGER := 2;
  ...
  PROCEDURE  PREPARE_REPLAY(
              ...
              capture_sts                IN BOOLEAN  DEFAULT FALSE,
              sts_cap_interval           IN NUMBER   DEFAULT 300,
              connect_time_auto_correct  IN BOOLEAN  DEFAULT TRUE,
  ...
  -- 
  --    Prerequisites:
  --      - DBMS_WORKLOAD_REPLAY.PREPARE_REPLAY has already been issued.
  --      - Enough external replay clients (WRC) have already been started. The
  --        client status can be monitored using V$WORKLOAD_REPLAY_CLIENTS. Use
  --        the WRC's CALIBRATE mode to determine the required number of replay
  --        clients.
  --      Example:
  ...
  --
  --    Prerequisites:
  --      - DBMS_WORKLOAD_REPLAY.START_REPLAY has already been issued.
  --
  -- ***********************************************************
  ...
  --    
  --    Parameters:
  --      reason        - an optional reason for cancelling the replay
  ...
  -- *********************************************************** 
  --  GET_REPLAY_INFO 
  --    Look in the given directory object and retrieve information about the 
  --    workload captures and the history of workload replays. 
  -- 
  --    The function skips loading the replay divergence data by default. To 
  --    load the data, set the parameter load_divergence to TRUE. 
  -- 
  --    Replay information can be deleted by calling DELETE_REPLAY_INFO.
  --
  --    Parameters: 
  --      replay_dir      - name of the directory object (case sensitive)
  --      load_details    - load the divergence and tracked commits data
  -- 
  --    RETURNS 
  --      the capture id for a single capture directory; 0 for a 
  --      consolidated capture directory 
  -- ***********************************************************
  FUNCTION GET_REPLAY_INFO(replay_dir      IN VARCHAR2,
  ...
  -- ***********************************************************
  FUNCTION GET_REPLAY_INFO(replay_dir      IN VARCHAR2,
                           load_details    IN VARCHAR2)
  RETURN NUMBER;
    
  -- *********************************************************** 
  --  DELETE_REPLAY_INFO 
  --    Delete information about one replay. If the parameter permanent is 
  --    TRUE, also delete the data from disk. 
  --
  --    Deleted replay information can be retrieved back by calling 
  --    GET_REPLAY_INFO if the replay data is not deleted from disk.
  -- 
  --    Parameters: 
  --      replay_id - ID of the workload replay that needs to be deleted 
  --      permanent - TRUE: delete data from disk; FALSE: don't delete data 
  --                  from disk 
  -- ***********************************************************
  ...
  -- *********************************************************** 
  --  DELETE_REPLAY_INFO 
  --    Delete information for all replays in the given directory object. If 
  --    the parameter permanent is TRUE, also delete the data from disk.
  --
  --    Deleted replay information can be retrieved back by calling 
  --    GET_REPLAY_INFO if the replay data is not deleted from disk.
  -- 
  ...
  --                   from disk 
  -- ***********************************************************
  PROCEDURE DELETE_REPLAY_INFO(replay_dir IN VARCHAR2,
                               permanent  IN BOOLEAN DEFAULT FALSE);
  ...
  -- ***********************************************************
  PROCEDURE LOAD_DIVERGENCE(replay_id IN NUMBER);
  
  -- *********************************************************** 
  --  LOAD_DIVERGENCE 
  --    Load divergence data for all replays in a given directory object.
  -- 
  --    Parameters: 
  --      replay_dir - name of the directory object (case sensitive)
  -- ***********************************************************  
  PROCEDURE LOAD_DIVERGENCE(replay_dir IN VARCHAR2);
  
  -- *********************************************************** 
  --  LOAD_TRACKED_COMMITS
  --    Load tracked commits data for a given replay id. 
  -- 
  --    Parameters: 
  --      replay_id - ID of the workload replay 
  -- ***********************************************************
  PROCEDURE LOAD_TRACKED_COMMITS(replay_id IN NUMBER);
  
  -- *********************************************************** 
  --  LOAD_TRACKED_COMMITS
  --    Load tracked commits data for all replays in a given 
  --    directory object.
  -- 
  --    Parameters: 
  --      replay_dir - name of the directory object (case sensitive)
  -- ***********************************************************  
  PROCEDURE LOAD_TRACKED_COMMITS(replay_dir IN VARCHAR2);
   ...
   *
   * Parameters:
   *   schedule_cap_id    - (IN) the id of the a capture in the schedule.
   *   capture_user       - (IN) the user name during the time of the
   *                             workload capture
   *   replay_user        - (IN) the user name to which captured user is
   *                             remapped during replay
   * Notes: - a schedule_cap_id of NULL is used for regular non-consolidated
   *          replay.
   ...
   *          the one with the schedule_cap_id parameter by passing in NULL
  ...
  --
  --    Generates a report that displays the results of Workload
  --    Intelligence.
  --
  --    Parameters:
  --      wi_job_name     - Name that uniquely identifies the Workload
  --                        Intelligence job.
  --      top_results     - A number that indicates the maximum number
  --                        of top patterns to appear in the report.
  --      format          - Specifies the report format
  --                        Valid values are
  --                        DBMS_WORKLOAD_CAPTURE.TYPE_HTML and
  --                        DBMS_WORKLOAD_CAPTURE.TYPE_XML
  ...
  --     RETURNS: the report
  -- ***********************************************************
  FUNCTION  WORKLOAD_INTELLIGENCE_REPORT(wi_job_name IN VARCHAR2,
  ...
  --   Get some information on a diverging call, including the statement 
  ...
  --   based on the parameter passed to CREATE_FILTER_SET() when creating
  --   the filter set.
  -- 
  --   *****************************
  --   SCOPE of the filter specified
  --   *****************************
  --   Filters once specified are valid only for the next successful
  ...
  --
  --    Parameters:
  --        replay_dir - replay directory object
  ...
  --   successful call to CREATE_FILTER_SET) to create a set of filters to 
   ...
   *   Set directory object to be used by a consolidated replay. This    
   *   procedure is deprecated. Use SET_CONSOLIDATED_DIRECTORY. 
   *
   * Parameters: 
   *   replay_dir - directory object pointing to an OS directory that contains 
   *                multiple captures for workload consolidation
   *                (case sensitive)
   ***********************************************************************/
   ...
   *   Set directory object to be used by a consolidated replay.
   *
   * Parameters: 
   *   replay_dir - directory object pointing to an OS directory that contains 
   *                multiple captures for workload consolidation
   *                (case sensitive)
   ***********************************************************************/
  PROCEDURE SET_CONSOLIDATED_DIRECTORY(replay_dir IN VARCHAR2);
   ...
   *                          (MANDATORY) 
   *   plsql_mode -(IN) Replay options for PL/SQL calls
   *	               'TOP_LEVEL': top-level PL/SQL only
   *	               'EXTENDED': SQL executed from PL/SQL or top-level PL/SQL
   *                              if there is no SQL recorded inside the PL/SQL
   *************************************************************************/
  ...
  /************************************************************************
   *  Assign a group of capture files to be processed by a particular node
   *  in RAC.
   *
   *  Parameters:
   *    group_id           - (IN) the identifier of the specified group of 
   *                              capture files
   *    instance_number    - (IN) the number used for instance registration.
   *                              It is equivalent to the 'instance_number'
   *                              column in v$instance.   
   *
   ***********************************************************************/
  PROCEDURE ASSIGN_GROUP_TO_INSTANCE(group_id        IN INTEGER,
                                     instance_number IN INTEGER);

  -- ***********************************************************
  -- GET_CLOCK
  --   Returns the current value of the replay clock
  --   via a kernel callout to kecpGetClock
  -- ***********************************************************
  FUNCTION GET_CLOCK RETURN NUMBER;
  ...
  FUNCTION PROCESS_REPLAY_GRAPH(parallel_level IN BINARY_INTEGER DEFAULT 1)
    RETURN NUMBER;

  /* Maximum amount of groups produced by process_capture/group_workload  */
  KEC_MAX_NUM_GROUPS        CONSTANT INTEGER := 32;

  /****************************************************************************
   * PROCEDURE GROUP_WORKLOAD
   *   Finds a grouping for the workload. First resolve all conflicts, i.e. put
   *   capture files with that share at least one object (non-read-only) into
   *   the same group. Afterwards merge groups based on the number of
   *   files. Until the number of groups is less than the max. number of groups
   *   The final result is a grouping where each group has approximately the
   *   same number of files. The results are stored in WRR$_WORKLOAD_GROUPS and
   *   WRR$_REPLAY_LOGIN_QUEUE.
   *
   * Parameters:
   *   max_groups - Maximum number of groups after grouping
   ***************************************************************************/
  PROCEDURE group_workload(max_groups IN INTEGER DEFAULT KEC_MAX_NUM_GROUPS);
  ...
  -- !!! THIS PROCEDURE IS DEPRECATED AND BE EVENTUALLY REMOVED !!! ---
  ...
  -- ************************************************************
  -- is my session a replay one?
  FUNCTION get_state RETURN BINARY_INTEGER;
  
  /************************************************************************
   * SET_SQL_MAPPING
   * This procedure specifies SQL statements to be skipped or replaced in 
   * replay
   *
   * Parameters
   *   schedule_cap_id - (IN) the id of a capture in the schedule
   *   sql_id          - (IN) SQL identifier of the SQL statement at the time
   *                          of capture
   *   operation       - (IN) it does one of the following in replay
   *                          'SKIP'   : skip this sql_id
   *                          'REPLACE': replace this sql_id with the SQL 
   *                                     statement in replacement_sql_text 
   *                                     parameter    
   *   replacement_sql_text - (IN) when 'SKIP', NULL. When 'REPLACE', 
   *                          SQL statement to be used
   ***********************************************************************/
  PROCEDURE SET_SQL_MAPPING(schedule_cap_id      IN NUMBER,
                            sql_id               IN VARCHAR2,
                            operation            IN VARCHAR2,
                            replacement_sql_text IN VARCHAR2 DEFAULT NULL);
  PROCEDURE SET_SQL_MAPPING(sql_id               IN VARCHAR2,
                            operation            IN VARCHAR2,
                            replacement_sql_text IN VARCHAR2 DEFAULT NULL);

  /************************************************************************
   * LOAD_LONG_SQLTEXT
   * This procedure loads the captured SQL statements whose length is greater
   * than 1000 characters. 
   *
   * Parameters
   *   capture_id - (IN) ID of the workload capture
   ***********************************************************************/
  PROCEDURE LOAD_LONG_SQLTEXT(capture_id IN NUMBER);
...
CREATE OR REPLACE PACKAGE dbms_wrr_state AS
  ...
  Capture_On CONSTANT BOOLEAN := ( dbms_workload_capture.get_state() = 1 );
  Replay_On  CONSTANT BOOLEAN := ( dbms_workload_replay.get_state()  = 1 );
END dbms_wrr_state;
/
GRANT EXECUTE ON dbms_wrr_state TO execute_catalog_role, dba
/
