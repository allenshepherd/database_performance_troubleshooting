Rem
Rem $Header: emll/admin/scripts/ocmdbb.sql /main/77 2014/09/17 09:20:35 dkuhn Exp $
Rem
Rem ocmdbb.sql
Rem
...
Rem    dkuhn       09/08/14 - higher version check
Rem    imunusam    03/17/14 - To fix security bug 18363985
Rem    pbhogara    08/23/13 - Fix bug 17347998
Rem    pbhogara    08/20/13 - match cell config columnsizes with GC
Rem    vasukuma    03/07/13 - bug 16118450 changes in emll
Rem    pbhogara    03/01/13 - increase size of makeModel column to 256
Rem    pbhogara    01/04/13 - match column sizes with GC for exadata targets
...
Rem    pbhogara    09/04/12 - convergence changes for exadata cell
...
BEGIN
  ...
  END IF;
  IF  l_temp_version= '12.2' THEN 
        return VERSION_12gR2; 
  END IF;
...
/*
   Below peocedure has been modified to fix bug 18363985.
   Instrumentation will be executed from from root container.
   so we don't need to switch to CDB$ROOT and it has been removed.
   Switching container was creating the bug.

   g_is_cdb will be set to 'YES' , if it is CDB and loggeg in to root container. 	
*/
...
BEGIN
  ...
  OR g_version_category = VERSION_12gR2
...
BEGIN
  ...
  ELSIF g_version_category = VERSION_10gR1 
     ...
     OR g_version_category = VERSION_11gR2 
     OR g_version_category = VERSION_12gR1
     OR g_version_category = VERSION_12gR2 THEN
    l_banner_split_clause :=
...
BEGIN
 ...
 l_sql := 'select cell.* ' ||
    ...
    '   interconnectCount number path ''interconnectCount'', ' ||
    '   iormBoost number path ''iormBoost'', '||
    '   ipaddress1 varchar2(20) path ''ipaddress1'', ' ||
    '   ipaddress2 varchar2(20) path ''ipaddress2'', ' ||
    '   ipaddress3 varchar2(20) path ''ipaddress3'', ' ||
    '   ipaddress4 varchar2(20) path ''ipaddress4'', ' ||
    '   kernelVersion varchar2(100) path ''kernelVersion'', ' ||
    '   makeModel varchar2(256) path ''makeModel'', ' ||
    '   metricHistoryDays number path ''metricHistoryDays'', ' ||
    '   powerCount varchar2(256) path ''powerCount'', ' ||
    '   releaseVersion varchar2(20) path ''releaseVersion'', ' ||
    '   snmpSubscriber varchar2(512) path ''normalize-space(substring(snmpSubscriber,1,512))'', ' ||
    '   smtpServer varchar2(100) path ''smtpServer'', ' ||
    '   smtpPort varchar2(10) path ''smtpPort'' ' ||
    ' ) cell where ' ||
...
procedure collect_gd_clients_config IS
  l_ip_cur INTEGER;
  l_ip_var VARCHAR2(256);
  l_res INTEGER;
  l_sql VARCHAR2(4000);
...
' with cellinfo as '||
' ( '||
   'SELECT cell.id, cc.cellname, cell.name '||
   'FROM v$cell_config cc, '||
        'xmltable( '||
        '''/cli-output/cell'' passing xmltype(cc.confval) '||
        ' columns '||
...
' griddisks as '||
' ( '||
    'select gds.name as diskname, gds.availableTo, ci.id, ci.name ' ||
    ' from ' ||
      ' v$cell_config cc, cellinfo ci,' ||
      ' xmltable ' ||
      ' (''/cli-output/griddisk'' passing xmltype(cc.confval) ' ||
          ' columns ' ||
            ' name varchar2(100) path ''name'', ' ||
            ' availableTo varchar2(256) path ''availableTo'' ' ||
      ' ) gds  where cc.conftype=''GRIDDISKS'' '||
               ' and ci.cellname = cc.cellname '||
               ' and ci.cellname='''|| l_ip_var || ''' '||
 ' ) '||
 'select gd.diskname, c.availableTo, gd.id, gd.name '||
 'from '||
  ' griddisks gd, '||
  ' xmltable( '||
     '''/clients/client'' passing xmltype(''<clients><client>'' || REPLACE(gd.availableTo, '','', ''</client><client>'') || ''</client></clients>'') '||
    ' columns '||
      ' availableTo varchar2(100) path ''text()'' '||
   ') c ';
    write_metric('cell_gd_clients_config', l_sql,
...
' ( '||
   ...
   ' WHERE cc.conftype = ''CELL'' '||
' ) '||
'select gds.name, gds.availableTo, gds.cellDisk, gds.gdcomment, ' ||
    'gds.creationTime, gds.errorCount, gds.id, round(gds.offset/1048576), ' ||
    'round(gds.gdsize/1048576), gds.status, ci.id, ci.name, gds.asmDiskGroupName, ' ||
    'gds.asmDiskName '||
' from ' ||
' v$cell_config cc, cellinfo ci, ' ||
' xmltable ' ||
...
' cds.raidLevel,round(cds.cdsize/1073741824, 2), cds.status, ci.id, ci.name ' ||
' from ' ||
   '  v$cell_config cc, cellinfo ci, ' ||
' xmltable ' ||
...
'  columns ' ||
 ' name varchar2(255) path ''name'', ' ||
 ' cdcomment varchar2(2000) path ''comment'', ' ||
 ' creationTime varchar2(30) path ''creationTime'', ' ||
 ' deviceName varchar2(255) path ''deviceName'', ' ||
 ' devicePartition varchar2(100) path ''devicePartition'', ' ||
 ' errorCount varchar2(256) path ''errorCount'', ' ||
 ' freeSpace varchar2(256) path ''freeSpace'', ' ||
 ' id varchar2(100) path ''id'', ' ||
 ' lun varchar2(255) path ''lun'', ' ||
 ' raidLevel varchar2(100) path ''raidLevel'', ' ||
 ' cdsize number path ''size'', ' ||
 ' status varchar2(256) path ''status'' ' ||
' ) cds  where cc.conftype=''CELLDISKS'' '||
       ' and cc.cellname = ci.cellname '||
       ' and ci.cellname='''|| l_ip_var || '''';
...
procedure collect_lun_physdisks_config IS
  l_ip_cur INTEGER;
  l_ip_var VARCHAR2(256);
  l_res INTEGER;
  l_sql VARCHAR2(4000);
...
' ( '||
  ...
  ' ), '||
'luns as (' ||
 ' select luns.name, luns.physicalDevices, ci.id ' ||
 ' from ' ||
   ' v$cell_config cc, cellinfo ci,' ||
 ' xmltable ' ||
 ' (''/cli-output/lun'' passing xmltype(cc.confval) ' ||
   ' columns ' ||
    ' name varchar2(255) path ''name'', ' ||
    ' physicalDevices varchar2(256) path ''physicalDrives'' ' ||
  ' ) luns where cc.conftype=''LUNS'' '||
     ' and cc.cellname = ci.cellname '||
     ' and ci.cellname='''|| l_ip_var || ''''||
 ') '||
 ' select luns.name, c.physicalDevice, luns.id '||
 ' from '||
  ' luns, '||
  ' xmltable( '||
     '''/pds/pd'' passing xmltype(''<pds><pd>'' || REPLACE(luns.physicalDevices, '','', ''</pd><pd>'') || ''</pd></pds>'') '||
    ' columns '||
    ' physicalDevice varchar2(255) path ''text()'' '||
  ') c ';

    write_metric('cell_lun_physdisks_config', l_sql,
    null, replace(l_ip_var, ':', '_'), '|' );
    END IF;
    END LOOP;
    DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      IF DBMS_SQL.IS_OPEN(l_ip_cur) THEN
         DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
      END IF;      
END collect_lun_physdisks_config;
...
' luns.raidLevel, luns.status, luns.lunUID, ci.id, cc.cellname ' ||
' from ' ||
   ' v$cell_config cc, cellinfo ci,' ||
' xmltable ' ||
' (''/cli-output/lun'' passing xmltype(cc.confval) ' ||
 ' columns ' ||
  ...
  ' lunAutoCreate varchar2(10) path ''lunAutoCreate'', ' ||
...
' REPLACE(pds.slotNumber,''"'',''''), pds.diskType, pds.makeModel, pds.physicalInterface, '||
' pds.physicalFirmware, pds.physicalSerial, ci.cellname, ci.id ' ||
' from ' ||
'     v$cell_config cc, cellinfo ci, ' ||
' xmltable ' ||
...
' ) pds where cc.conftype like ''PHYSICAL%'' '||
   ' and cc.cellname = ci.cellname '||
   ' and cc.cellname='''|| l_ip_var || '''';
...
procedure collect_pdisk_luns_config IS
  l_ip_cur INTEGER;
  l_ip_var VARCHAR2(256);
  l_res INTEGER;
  l_sql VARCHAR2(4000);
...
' ), '||
' pds as ( '||
' select pds.slotNumber, pds.luns, ci.id, pds.name ' ||
' from ' ||
'     v$cell_config cc, cellinfo ci, ' ||
' xmltable ' ||
' (''/cli-output/physicaldisk'' passing xmltype(cc.confval) ' ||
...
' ) pds where cc.conftype like ''PHYSICAL%'''||
  ...
  ' and cc.cellname='''|| l_ip_var || '''' ||
' ) '||
 ' select REPLACE(pds.slotNumber,''"'',''''), c.lun, pds.id, pds.name '||
 ' from pds, '||
      ' xmltable( '||
        '''/luns/lun'' passing xmltype(''<luns><lun>'' || REPLACE(pds.luns, '','', ''</lun><lun>'') || ''</lun></luns>'') '||
      ' columns '||
      ' lun varchar2(255) path ''text()'' '||
      ' ) c ';

    write_metric('cell_physicaldisk_luns_config', l_sql,
    null, replace(l_ip_var, ':', '_'), '|' );
    END IF;
    END LOOP;
    DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      IF DBMS_SQL.IS_OPEN(l_ip_cur) THEN
         DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
      END IF;      
END collect_pdisk_luns_config;


procedure collect_iorm_dbplan_config IS
    l_ip_cur INTEGER;
    l_ip_var VARCHAR2(256);
    l_res INTEGER;
    l_sql VARCHAR2(4000);
...
' and cc.cellname='''|| l_ip_var || ''''||
' ),  '||
' percentageplanoncells as(  '||
' SELECT  cc.cellname, ''Yes'' ispercentagebasedplan FROM  '||
'                           v$cell_config cc,  '||                         
'                           XMLTable(''for $i in /cli-output/interdatabaseplan  '||
'                                     for $j in $i/dbPlan/directive  '||   
'                                      return  '||
'                                         <result>  '||                                            
'                                             {$j}  '||
'                                         </result> ''  '||
'                                     PASSING xmltype(cc.confval)  '||
'                                     COLUMNS dbname   VARCHAR2(255) PATH ''/result/directive/@name'') dbp  '||
' WHERE cc.conftype = ''IORM''  '||
' AND dbp.dbname = ''other''  '||
' and cc.cellname='''|| l_ip_var || ''''||
' ),  '||
' dbplan as(  '||
' SELECT  cc.cellname, dbp.dbname, dbp.planname, dbp.alloclevel,  '||
'          dbp.alloc, dbp.shareval, dbp.limit, dbp.role, dbp.flashcache, dbp.flashlog  '||
'  FROM v$cell_config cc, XMLTable(''for $i in /cli-output/interdatabaseplan  '||
'                                     for $j in $i/dbPlan/directive     '||
'                                      return  '||
'                                         <result>  '||
'                                             <name>{$i/name}</name>  '||
...
'                                         </result> ''  '||
'                                     PASSING xmltype(cc.confval)  '||
'                                     COLUMNS planname VARCHAR2(255) PATH ''/result/name'',  '||
'                                             dbname   VARCHAR2(255) PATH ''/result/directive/@name'',  '||
'                                             alloclevel   NUMBER  PATH ''/result/directive/@level'',  '||
'                                             alloc VARCHAR2(256) PATH ''/result/directive/@allocation'',  '||
'                                             shareval NUMBER PATH ''/result/directive/@share'',  '||
'                                             flashcache VARCHAR2(5) PATH ''/result/directive/@flashcache'',  '||
'                                             flashlog   VARCHAR2(5) PATH ''/result/directive/@flashlog'',  '||
'                                             role  VARCHAR2(15) PATH ''/result/directive/@role'',  '||
'                                             limit VARCHAR2(256) PATH ''/result/directive/@limit''  '||
'                                          ) dbp  '||
' WHERE cc.conftype = ''IORM''  '||
' and cc.cellname='''|| l_ip_var || ''''||
' )  '||
' SELECT  dbname, cc.id, planname, alloclevel,  '||
'          alloc, shareval, limit, role, flashcache, flashlog,   '||
'          decode(ispercentagebasedplan, NULL, ''No'', ''Yes'')ispercentagebasedplan    '||
' FROM (SELECT dbp.cellname, dbp.dbname, dbp.planname, dbp.alloclevel,  '||
'               dbp.alloc, dbp.shareval, dbp.limit, dbp.role, dbp.flashcache,   '||
'               dbp.flashlog, ppcells.ispercentagebasedplan   '||
'               FROM dbplan dbp   '||
'               LEFT JOIN percentageplanoncells ppcells  '||
'               ON dbp.cellname = ppcells.cellname) interdatabaseplan   '||
' LEFT JOIN cellinfo cc  '||
' ON cc.cellname=interdatabaseplan.cellname ';

    write_metric('cell_iorm_dbplan_config', l_sql,
    null, replace(l_ip_var, ':', '_'), '|' );
    END IF;
    END LOOP;
    DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      IF DBMS_SQL.IS_OPEN(l_ip_cur) THEN
         DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
      END IF;
END collect_iorm_dbplan_config;


procedure collect_iorm_catplan_config IS
    l_ip_cur INTEGER;
    l_ip_var VARCHAR2(256);
    l_res INTEGER;
    l_sql VARCHAR2(4000);
...
' ( '||
  ...
  ' WHERE cc.conftype = ''CELL'' '||
' ) '||
' SELECT  catp.catname, ci.id, catp.planname, catp.alloclevel, catp.allocation '||
' FROM v$cell_config cc, XMLTable(''for $i in /cli-output/interdatabaseplan '||
                                   ' for $j in $i/catPlan/directive   '||
                                    ' return '||
                                       ' <result> '||
                                           ' <name>{$i/name}</name> '||
                                           ' {$j} '||
                                    ...
                                    ' PASSING xmltype(cc.confval) '||
                                    ' COLUMNS planname VARCHAR2(255) PATH ''/result/name'', '||
                                            ' catname  VARCHAR2(255) PATH ''/result/directive/@name'', '||
                                            ' alloclevel   NUMBER  PATH ''/result/directive/@level'', '||
                                            ' allocation VARCHAR2(256) PATH ''/result/directive/@allocation'' '||
                                  ') catp, cellinfo ci '||
' WHERE cc.conftype = ''IORM'' '||
  ' and cc.cellname = ci.cellname '||
  ' and cc.cellname='''|| l_ip_var || '''';
    write_metric('cell_iorm_catplan_config', l_sql,
    null, replace(l_ip_var, ':', '_'), '|' );
    END IF;
    END LOOP;
    DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      IF DBMS_SQL.IS_OPEN(l_ip_cur) THEN
         DBMS_SQL.CLOSE_CURSOR(l_ip_cur);
      END IF;
END collect_iorm_catplan_config;
...
'   SELECT XMLQuery( '||
       ' ''string-join '||
            ' ( '||
              ' for $i in /cli-output/interdatabaseplan/dbPlan/directive '||
              ' return '||
              ' ( '||                                 
              '  concat(concat(substring(concat("NA", $i/@name), 3 * number(boolean($i/@name))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@level), 3 * number(boolean($i/@level))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@allocation), 3 * number(boolean($i/@allocation))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@share), 3 * number(boolean($i/@share))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@flashcache), 3 * number(boolean($i/@flashcache))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@flashlog), 3 * number(boolean($i/@flashlog))),"#@#"), '||
              '  concat(substring(concat("NA", $i/@role), 3 * number(boolean($i/@role))),"#@#"), '||
              '  substring(concat("NA", $i/@limit), 3 * number(boolean($i/@limit)))) '||
              ' ), '||
              ' "[@]" '||
             ')''' ||
           ' PASSING xmltype(cc.confval) '||
           ' returning content) as dbPlan, cc.cellname '||
  ' FROM v$cell_config cc '||
  ' WHERE cc.conftype =''IORM'' '||
' ), '||
' catPlanView as ' ||
' ( ' ||
  ...
  ' ''string-join '||
           ' ( '||
              ' for $i in /cli-output/interdatabaseplan/catPlan/directive '||
              ' return '||
              ' ( '||
               ' concat(concat(substring(concat("NA", $i/@name), 3 * number(boolean($i/@name))),"#@#"), '||
               ' concat(substring(concat("NA", $i/@level), 3 * number(boolean($i/@level))),"#@#"), '||
            ...
            ' )''' ||
           ' PASSING xmltype(cc.confval) '||
           ' returning content) as catPlan, cc.cellname '||
 ' FROM v$cell_config cc '||
 ' WHERE cc.conftype =''IORM'' '||
' ) '||
' select iorm.name, cp.catPlan, '||
       ' dbp.dbPlan, iorm.status, '||
       ' iorm.objective, ci.id, cc.cellname '||
' from v$cell_config cc, '||
    '  xmltable '||
     ' (''/cli-output/interdatabaseplan'' passing xmltype(cc.confval) '||
      ' columns '||
       ' name varchar2(255) path ''name'', '||
       ' catPlan varchar2(4000) path ''catPlan'', '||
       ' status varchar2(15) path ''status'', '||
       ' objective varchar2(20) path ''objective'' '||
      ') iorm, '||
  'dbPlanView dbp, catPlanView cp,'||
  'cellinfo ci '||
' where cc.conftype =''IORM'' '||
'  and cc.cellname = dbp.cellname '||
'  and cc.cellname = cp.cellname '||
'  and cc.cellname = ci.cellname '||
'  and cc.cellname='''|| l_ip_var || '''';
...
BEGIN
  ...
  l_sql := 'select cell.* ' ||
    ...
    ' (''/cli-output/cell'' passing xmltype(cc.confval) ' ||
    ' columns ' ||
    '   releaseTrackingBug varchar2(20) path ''releaseTrackingBug'', ' ||
    '   id varchar2(100) path ''id'' ' ||
    ' ) cell where ' ||
    '   cc.conftype=''CELL'' and cellname='''|| l_ip_var || '''';
...
BEGIN
  ...
  IF g_version_category = VERSION_10gR1 OR
   ...
   collect_gd_clients_config;
   collect_celldisk_config;
   collect_lun_config;
   collect_lun_physdisks_config;
   collect_physicaldisk_config;
   collect_pdisk_luns_config;
   collect_iorm_config;
   collect_iorm_dbplan_config;
   collect_iorm_catplan_config;
   collect_patches;
