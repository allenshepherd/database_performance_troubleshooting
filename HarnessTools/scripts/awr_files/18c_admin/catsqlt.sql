Rem
Rem $Header: rdbms/admin/catsqlt.sql /main/37 2016/08/10 11:31:00 aarvanit Exp $
Rem
Rem catsqlt.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catsqlt.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catsqlt.sql
Rem SQL_PHASE: CATSQLT
...
Rem    aarvanit    06/14/16 - bug #23144337: sqlset_row: NULL default con_dbid
Rem    aarvanit    10/19/15 - RTI #13227832: modify WRI$_SQLSET_PLAN_LINES
Rem                           column datatypes to match with staging table
Rem    raeburns    09/21/15 - bug 21864109: sql_bind and sqlprof_attr used 
Rem                           in tables in GSI db.
Rem    raeburns    05/31/15 - Use FORCE for types with only type dependents
Rem    hpoduri     03/12/15 - proj: 47331, add wri$_index_usage table
Rem    prakumar    02/03/15 - Bug 20431667: Support for long iden in sqlset_row
Rem    bhavenka    07/21/14 - 9853147: add last_exec_start_time to sqlset_row
Rem                           and wri$_sqlset_statistics
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    shjoshi     03/12/12 - Add flags and masked_binds_flags to
Rem                           wri$_sqlset_plans
Rem    kyagoub     04/01/11 - add con_dbid to sqlset_row
...
--     Use create type, so customers can use this type in tables without
--     encountering upgrade errors.  Use ALTER TYPE for any future changes.
--------------------------------------------------------------------------------
CREATE TYPE sql_binds AS VARRAY(2000) OF ANYDATA
/
...
--
--     bug 21864109: sql_bind used in table in GSI db, so future changes to the 
--     type will need to be made with ALTER TYPE statements, after all dependent
...
--------------------------------------------------------------------------------
CREATE OR REPLACE TYPE sqlset_row FORCE AS object (
  -- 
  ...
  parsing_schema_name      VARCHAR2(128),   /* schema where the SQL is parsed */
  ...
  con_dbid                 NUMBER,           /* dbid of the pluggable/root db */
  last_exec_start_time     VARCHAR2(19),  /* last exec start time of this sql */
  --
  ...
  CONSTRUCTOR FUNCTION sqlset_row(  
    ...
    con_dbid                 NUMBER              := NULL,
    last_exec_start_time     VARCHAR2            := NULL)
...
--
-- PRIMARY KEY:
--     The primary key is (stmt_id, con_id). The statement id, or for short 
--     stmt_id, is an internal key generated using the
--     wri$_sqlset_stmt_id_seq sequence. A new stmt_id is created for every new
--     (sqlset_id, sql_id, objects_hash_value, con_dbid).
--
-- FOREIGN KEYS:
--      SQLSET_ID references WRI$_SQLSET_DEFINITIONS(ID)
--------------------------------------------------------------------------------
...
(
  ...
  action                   VARCHAR2(64),
  command_type             NUMBER,
  constraint wri$_sqlset_statements_pk primary key (id, con_dbid)
  using INDEX tablespace SYSAUX
) 
tablespace SYSAUX
/
...
-- With con_dbid, since sqlset_id, sql_id are no longer unique, include
-- con_dbid as well
create unique index wri$_sqlset_statements_idx_01 
on wri$_sqlset_statements(sqlset_id, sql_id, con_dbid) 
tablespace sysaux
/

-- create an index for the force_matching_signature
-- With con_dbid, since sqlset_id, force_matching_signature are no longer 
-- unique, include con_dbid as well
create index wri$_sqlset_statements_idx_02 
on wri$_sqlset_statements(sqlset_id, force_matching_signature, con_dbid) 
tablespace sysaux
/
...
--
-- PRIMARY KEY:
--     The primary key is (STMT_ID, PLAN_HASH_VALUE, CON_DBID).
--
-- FOREIGN KEYS:
--     (STMT_ID, CON_DBID) references WRI$_SQLSET_STATEMENTS(ID, CON_DBID)
--------------------------------------------------------------------------------
...
--
--  PRIMARY KEY:
--     (STMT_ID, PLAN_HASH_VALUE, CON_DBID)
--------------------------------------------------------------------------------
...
--
-- PRIMARY KEY:
--     The primary key is (STMT_ID, PLAN_HASH_VALUE, ID, CON_DBID).
--
-- FOREIGN KEYS:
...
-- 
-- PRIMARY KEY:
--     STMT_ID, PLAN_HASH_VALUE, POSITION, COND_DBID
--
-- FOREIGN KEY:
...
-- xml in the code.
--
-- bug 21864109: sqlprof_attr used in tables in GSI db; any future changes 
-- need to be made with ALTER TYPE statements

-- 
CREATE TYPE sys.sqlprof_attr
...
--
--  DESCRIPTION:
--     This table tracks index usage in the database.
--
--  PRIMARY KEY:
--     OBJ#
--
...
create table wri$_index_usage(
 ...
 spare4                            NUMBER,
 spare5                              DATE,
 constraint wri$_index_usage_pk 
 primary key (obj#)
 using INDEX tablespace SYSAUX
)
tablespace SYSAUX 
