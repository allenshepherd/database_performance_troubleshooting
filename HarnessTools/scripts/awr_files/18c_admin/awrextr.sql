Rem
Rem $Header: rdbms/admin/awrextr.sql /main/8 2017/05/28 22:46:00 stanaya Exp $
Rem
Rem awrextr.sql
Rem
...
Rem    SQL_SOURCE_FILE: rdbms/admin/awrextr.sql
Rem    SQL_SHIPPED_FILE: rdbms/admin/awrextr.sql
...
Rem    arbalakr    08/23/16 - Bug 24506955: Choose local dbid from sys_context
Rem                           and not v$database.
...
select sys_context('userenv','con_dbid')           db_dbid
     , d.name           db_name
  from v$database d;
...
select distinct
     ...
     , wr.db_name         dbb_name
     , wr.host_name       host
  from awr_pdb_database_instance wr, v$database cd
  order by dbbid desc;
...
begin
  ...
  if cidnum%notfound then
    raise_application_error(-20200,
      'Database ' || :dbid || 
      ' does not exist in AWR_PDB_DATABASE_INSTANCE');
  end if;
  close cidnum;
...
begin
  ...
  dbms_workload_repository.extract(dmpfile  => :dmpfile,
                                   dmpdir   => :dmpdir,
                                   bid      => :bid,
                                   eid      => :eid,
                                   dbid     => :dbid);
