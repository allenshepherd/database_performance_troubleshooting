Rem
Rem $Header: rdbms/admin/dbmsstr.sql /main/77 2017/07/31 15:02:02 jorgrive Exp $
Rem
Rem dbmsstr.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/dbmsstr.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/dbmsstr.sql
Rem SQL_PHASE: DBMSSTR
...
Rem    sramakri    12/20/16 - lrg-19980009: remove prvthcdc.plb
Rem    fergutie    08/01/15 - Bug 24325339, 24324702, 24325184
Rem    huntran     02/25/15 - prvthggei
Rem    jorgrive    10/20/14 - Desupport Advanced Replication
Rem    jorgrive    07/09/14 - Add dbms_streams_adm.delete_replication_events
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    tianli      07/23/13 - bug 12816679: fix remove_streams_configuration
Rem    yuli        05/10/13 - add compatible_12_2
Rem    myuin       12/26/12 - XbranchMerge myuin_bug-15981425_12.1.0.1.0 from
Rem                           st_rdbms_12.1.0.1
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    yurxu       09/09/10 - add compatible_12_1
...
CREATE OR REPLACE PACKAGE dbms_streams AUTHID CURRENT_USER AS
  ...
  -- internal compatible representation for 12.1
  FUNCTION compatible_12_1 RETURN INTEGER;

  -- internal compatible representation for 12.2
  FUNCTION compatible_12_2 RETURN INTEGER;
 ...
 -- get_final_lob was moved from dbms_reputil2 
 ...
 FUNCTION get_final_lob(column_changed$_char     IN CHAR,
  ...
  -- moved from dbms_repcat_utl
  -- If user exists, return id
  -- else return null
  FUNCTION convert_user_to_id(canon_uname IN VARCHAR2) RETURN NUMBER;
...
PROCEDURE remove_streams_configuration(
  streams_only IN BOOLEAN DEFAULT TRUE);
...
--                comma-separated list.
  ...
  --  include_error : include error events, default true.
...
CREATE OR REPLACE PACKAGE dbms_streams_advisor_adm AUTHID CURRENT_USER AS  
  ...
  extract_type              CONSTANT NUMBER := 6;
  replicat_type             CONSTANT NUMBER := 7;
  ...
  FUNCTION  get_retention_time RETURN NUMBER;
  PROCEDURE set_retention_time (r_time NUMBER);

  FUNCTION  get_bottleneck_idle_threshold RETURN NUMBER;
  PROCEDURE set_bottleneck_idle_threshold (threshold NUMBER);

  FUNCTION  get_bottleneck_flowctrl_thresh RETURN NUMBER;
  PROCEDURE set_bottleneck_flowctrl_thresh (threshold NUMBER);
...
Rem Private GoldenGate export header
@@prvthggei.plb

Rem Private GoldenGate header
@@prvthgg.plb
