Rem
Rem $Header: rdbms/admin/catadvtb.sql /main/73 2017/09/14 09:25:44 msabesan Exp $
Rem
Rem catadvtb.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catadvtb.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catadvtb.sql
Rem SQL_PHASE: CATADVTB
...
Rem    msabesan    08/29/17 - bug 26712379: add cached gets and direct gets
Rem    victlope    01/19/17 - Proj 70435: Add concurrent exec column
Rem    jiayan      08/03/15 - #20663978: add rec_type_id column
Rem                           to wri$_adv_recommendations
Rem    raeburns    05/31/15 - remove OR REPLACE for types with table dependents
Rem    cmlim       10/16/14 - bug 19791377: remove flush shared pool in
Rem                           parallel phase for upgrade performance
Rem    jiayan      09/15/14 - proj 44162: create Stats Advisor object type
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    mfallen     10/17/12 - bug 14390165: add con_dbid
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    ddas        08/22/11 - SPM Evolve Advisor
Rem    kyagoub     03/30/11 - project-35499: add attrXXX columns to adv_objects
Rem    traney      03/27/11 - 35209: long identifiers dictionary upgrade
...
CREATE TYPE wri$_adv_abstract_t AS OBJECT 
(
  advisor_id      number,                        
...
(
 ...
 name            varchar2(128)   not null,                           /* name */
...
create table wri$_adv_def_parameters
   (
      ...
      exec_type   varchar2(128),/* exe. action the parameter can be set for */
...
create table wri$_adv_def_exec_types
   (
      advisor_id  number        not null,              /* advisor id number */
      name        varchar2(128) not null,          /* execution action name */
...
( 
 ...
 last_exec_name       varchar2(128),       /* last exec. id as an optimizaton */
 ...
 source               varchar2(128),           /* optional name of base task */
...
( 
 ...
 name               varchar2(128)   not null,   /* unique name for execution */
 description        varchar2(256),         /* optional execution description */
 exec_type          varchar2(128),    /* type of the exec. action to perform */
 ...
 req_dop            number,         /* requested dop of concurrent execution */
 actual_dop         number,   /* actual dop used during concurrent execution */
...
(
 ...
 exec_name      varchar2(128)   not null,                 /* executuion name */
 name           varchar2(128)   not null,                  /* parameter name */
 value          varchar2(4000)  not null,                 /* parameter value */
...
(
 ...
 exec_name      varchar2(128),                     /* optional execution id. */
...
(
 ...
 task_id        number          not null,                  /* associate task */
 exec_name      varchar2(128),          /* accociated execution id: optional */
...
(
  ...
  domain          varchar2(128) not null,         /* Domain or namespace name */
  name            varchar2(128) not null,                   /* Secondary name */
...
(
  ...
  name            varchar2(128) not null,       /* User supplied name.  Must */
...
create table wri$_adv_journal
   (
      ...
      exec_name            varchar2(128),        /* id of the task execution */
...
(
 ...
 exec_name      varchar2(128),            /* if of the asscociated execution */
...
(
  ...
  exec_name         VARCHAR2(128)  NOT NULL,
...
(
  ...
  spare_n9                       NUMBER,
  spare_n10                      NUMBER,
  cached_gets                    NUMBER,  /* cached gets */
  direct_gets                    NUMBER,  /* direct reads */
...
CREATE GLOBAL TEMPORARY TABLE wri$_adv_addm_pdbs 
(
...
CREATE TYPE wri$_adv_sqlaccess_adv under wri$_adv_abstract_t
  (
...
CREATE TYPE wri$_adv_tunemview_adv under wri$_adv_abstract_t
  (
    overriding member procedure sub_execute(task_id in NUMBER,
                                            err_num out number),
    overriding member procedure sub_reset(task_id in number),
...
--
-- DESCRIPTION: 
--     This is a sub-type object type that implements the SQL performance
--     change impact analysis advisor.
--     This type is created under the sqltune object type so that it can
--     inherit all sqltune methods.
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
--                   wri$_adv_spm_evolve sub type definition                 --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- NAME:
--     wri$_adv_spm_evolve
--
-- DESCRIPTION:
--     This is a sub-type object type that implements the SPM Evolve Advisor.
--     It is mainly used to integrate the advisor within the Advisor Framework.
--     The implementation of this type resides in .../sqlman/spm/prvtspmi.sql
-------------------------------------------------------------------------------
create type wri$_adv_spm_evolve under wri$_adv_abstract_t
(
  overriding MEMBER PROCEDURE sub_execute(task_id IN NUMBER,
                                          err_num OUT NUMBER),
  overriding MEMBER PROCEDURE sub_reset(task_id IN NUMBER),
...
-------------------------------------------------------------------------------
--                   wri$_adv_stats sub type definition                      --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- NAME:
--     wri$_adv_stats
--
-- DESCRIPTION:
--     This is a sub-type object type that implements the Stats Advisor.
--     It is mainly used to integrate the advisor within the Advisor Framework.
--     The implementation of this type resides in .../stat/prvtstatadvi.sql
-------------------------------------------------------------------------------
