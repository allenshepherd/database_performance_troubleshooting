Rem
Rem $Header: plsql/admin/utli18n.sql /main/11 2017/07/27 17:28:36 qma Exp $
Rem
Rem utli18n.sql
Rem
...
Rem SQL_SOURCE_FILE: plsql/admin/utli18n.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/utli18n.sql
Rem SQL_PHASE: UTLI18N
...
Rem    qma         04/28/17 - Add validate_character_encoding
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    attran      12/12/11 - updated header comment for GET_MAX_CHARACTER_SIZE
Rem    attran      10/11/11 - added GET_MAX_CHARACTER_SIZE
...
RETURN VARCHAR2 CHARACTER SET param1%CHARSET;
   ...
   *
   * PARAMETERS
   *   charset_name  A case-insensitive but valid character set name
   *
   * RETURN
   *   The max character size
   */
FUNCTION get_max_character_size(charset_name IN VARCHAR2)
RETURN PLS_INTEGER;

  /**
   * Validate VARCHAR2/NVARCHAR2 data for database/national character set.
   *
   * The function validates the character encoding of VARCHAR2/NVARCHAR2
   * data. The validation is based on
   * the database character set for VARCHAR2 and the national character
   * set for NVARCHAR2. It returns the offset of the first invalid byte
   * or 0 if all the bytes are valid. Note, the offset is 1-based, which is
   * the convention used by SQL functions such as INSTRB().
   *
   * A byte or byte sequence is considered invalid for a character set if it
   * cannot be mapped to any character defined in the character set. For
   * Unicode character sets (AL32UTF8, AL16UTF16, AL16UTF16LE, UTF8, and UTFE),
   * any byte sequences mapped to the following Unicode code points are
   * also considered invalid:
   *
   *   1. Unpaired surrogate code point
   *   2. Noncharacter code point
   *
   * In addition, any irregular or illegal UTF-8 byte sequence is considered
   * invalid for AL32UTF8 or UTF8 character set.
   *
   * PARAMETERS
   *   data        The input VARCHAR2/NVARCHAR2 data to validate.
   * RETURN
   *   0 if the VARCHAR2/NVARCHAR2 data does not contain any invalid bytes
   *   or the offset of the first invalid byte in the data. Return NULL if data
   *   is NULL.
   * EXCEPTIONS
   *   Miscellaneous runtime exceptions.
   */
FUNCTION validate_character_encoding(str IN VARCHAR2 CHARACTER SET ANY_CS)
RETURN PLS_INTEGER;

  /**
   * Validate CLOB/NCLOB data for database/national character set.
   *
   * The function validates the character encoding of CLOB/NCLOB
   * data. The validation is based on
   * the database character set for CLOB and the national character
   * set for NCLOB. It returns the offset of the first invalid character
   * or 0 if all the characters are valid. Note, the offset is 1-based,
   * which is the convention used by SQL functions such as SUBSTR().
   *
   * A LOB character is considered invalid for a character set if the byte
   * (in case of a single-byte database character set) or the byte pair
   * (in case of the UTF-16 encoding used with multibyte database
   * character sets) corresponding to the encoding of the character
   * does not map to any
   * character defined in the character set. For Unicode character sets
   * (AL32UTF8, AL16UTF16, AL16UTF16LE, UTF8, UTFE), any byte sequences
   * mapped to the following Unicode code points are also considered invalid:
   *
   *   1. Unpaired surrogate code point
   *   2. Noncharacter code point
   *
   * In addition, any ill-formed UTF-8 byte sequence is considered invalid for
   * the AL32UTF8 character set and any ill-formed CESU-8 byte sequence is
   * considered invalid for the UTF8 character set.
   *
   * PARAMETERS
   *   lob_loc        Locator for the CLOB/NCLOB to be validated.
   * RETURN
   *   0 if the CLOB/NCLOB does not contain any invalid characters or
   *   the offset of the first invalid character in the CLOB/NCLOB.
   *   Return NULL if lob_loc is NULL.
   * EXCEPTIONS
   *   ORA-22279 if buffering is enabled for lob_loc.
   */
FUNCTION validate_character_encoding(lob_loc IN CLOB CHARACTER SET ANY_CS)
RETURN NUMBER;
