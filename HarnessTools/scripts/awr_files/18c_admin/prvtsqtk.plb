...
begin
  ...
  hm_sqltk_internal.column_check('tab$','obj#','< 0',
        'Obj# invalid - < 0');
  hm_sqltk_internal.ref_check('tab$','file#','file$','relfile#',
        'file# != 0 and file# != 1024',
        'foreign key relfile# not found in file$');
  ...
  hm_sqltk_internal.column_check
        ('tab$','pctfree$','not between 0 and 99 and bitand(flags, 268435456) != 268435456',
...
begin
 ...
 hm_sqltk_internal.ref_check('clu$','file#','file$','relfile#', 'file# != 1024',
        'Foreign Key - clu$.file# not found in file$.relfile#');
 ...
 hm_sqltk_internal.ref_check
  ...
  hm_sqltk_internal.column_check
        ('clu$','func','not in (0,2,3)','Invalid function found - not in (0,2,3)');
...
begin
  ...
  hm_sqltk_internal.column_check
        ('ts$','online$','not between 1 and 4', 'ts$.online$');
  hm_sqltk_internal.tab_desc
        ('ts$',
...
begin
  ...
  hm_sqltk_internal.column_check
        ('file$','ts#','is not null and ts# not in (select ts# from ts$)','file$.status$');
  hm_sqltk_internal.column_check
...
begin
  ...
  hm_sqltk_internal.column_check
        ('obj$','namespace','not between 1 and 114','obj$.namespace');
  hm_sqltk_internal.column_check
        ('obj$','type#','not between 0 and 137', 'obj$.type#');
  hm_sqltk_internal.column_check
...
begin
  ...
  hm_sqltk_internal.column_check
        ('user$','astatus',
         'not in (select status# from sys.user_astatus_map) and (mod(astatus, 16) not in (select status# from sys.user_astatus_map))',
         'user$.astatus');
  hm_sqltk_internal.tab_desc
        ('user$',
...
begin
  ...
  hm_sqltk_internal.column_check
        ('seq$','partcount',
         'is not null and partcount not between 0 and 100000000',
         'check seq$.partcount range');
 commit;
end;
/
begin
...
begin
  ...
  hm_sqltk_internal.row_check
        ('sysauth$','grantee#,privilege#,sequence#','primary key',
                'sysauth$ pk');
  hm_sqltk_internal.ref_check
        ('sysauth$','grantee#','user$','user#',
         'grantee# < (select user# from user$ where name = ''_NEXT_USER'')',
         'sysauth$.grantee# fk');
  hm_sqltk_internal.column_check
...
begin
  ...
  hm_sqltk_internal.ref_check
        ('ecol$','tabobj#,colnum','col$','obj#,intcol#', '',
