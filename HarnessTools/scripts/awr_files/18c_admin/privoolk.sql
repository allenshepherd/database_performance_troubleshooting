...
Rem     cvanes     05/09/17  - 25973075: unqualified reference in dynamic sql
Rem     vtropash   04/27/16  - bug 23185746 fix
Rem     vtropash   12/02/15  - 22100537
Rem     vtropash   08/26/15  - 21683691
Rem     vtropash   07/16/15  - Bug 20289384
Rem     howu       05/08/08  - add DBMS_ASSERT.ENQUOTE_NAME to prevent SQL
Rem                            injection
Rem     howu       05/08/08  - XbranchMerge
Rem                            rpang_bug-5844738_10.1.2.0.0rdbms.6.1 from
Rem                            st_www_101200rdbms
...
as
   ...
   is
      ...
      rc          number;
      found       number := 0;
      p_schema    varchar2(1111) := upper(p_owner);
      p_obj       varchar2(1111) := upper(p_object);
   begin
   ...
   begin
      ...
      loop
         ...
         if (l_status <= 0) then
          ...
          if (l_data_type != 'XMLTYPE') then
            ...
            l_theQuery := l_theQuery || 'xmltype.getclobval(' || 
                          DBMS_ASSERT.ENQUOTE_NAME(l_column_name) || ')';
          end if; 
      end loop;
      ...
      l_cursor := dbms_sql.open_cursor;
      sys.dbms_sql.parse( l_cursor, l_theQuery, dbms_sql.v7);
   ...
   is
      ...
      p_schema      varchar2(1111) := upper(p_old_values(1));
      p_obj         varchar2(1111) := upper(p_old_values(2));
   begin
      -- Verify that there is no SQL injection
      validate_object_name (p_schema, p_obj);
