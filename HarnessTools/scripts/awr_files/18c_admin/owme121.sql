...
@@owmr1210.plb
begin
  for nt_rec in (select owner, table_name from wmsys.wm$nested_columns_table) loop
    execute immediate 'revoke all on ' || nt_rec.owner || '.' || nt_rec.table_name || '_LT from wmsys' ;
  end loop ;
end;
/
declare
