Rem
Rem $Header: rdbms/admin/xse111.sql /main/8 2017/05/28 22:46:14 stanaya Exp $
Rem
Rem xse111.sql
Rem
...
Rem    SQL_SOURCE_FILE: rdbms/admin/xse111.sql
Rem    SQL_SHIPPED_FILE: rdbms/admin/xse111.sql
...
Rem    minx        10/15/12 - lrg 7323743: check the xml resource exists
Rem                           before deleting it
Rem    snadhika    11/15/10 - Remove proxy related view
Rem    snadhika    04/14/10 - remove PREDICATE xmlindex
Rem    rpang       03/30/09 - downgrade PL/SQL network ACL security objects
Rem    rbhatti     12/05/08 - Not drop v$xs_session
Rem    yiru        03/13/08 - add 11.2 downgrade
...
Rem    srseshad    12/02/07 - Drops related to lightweight proxy
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/securityclasses/xssystemsc.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/securityclasses/xssystemsc.xml',DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/roles/xspublic.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/roles/xspublic.xml',DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/users/xsguest.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/users/xsguest.xml', DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/roles/xsauthenticated.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/roles/xsauthenticated.xml', DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/roles/dbms_auth.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/roles/dbms_auth.xml', DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/roles/dbms_passwd.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/roles/dbms_passwd.xml', DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
...
BEGIN
  if (DBMS_XDB.existsResource('/sys/xs/roles/midtier_auth.xml')) then
    DBMS_XDB.deleteResource('/sys/xs/roles/midtier_auth.xml', DBMS_XDB.DELETE_RECURSIVE_FORCE);
  end if;
EXCEPTION 
  when others then
  NULL;
