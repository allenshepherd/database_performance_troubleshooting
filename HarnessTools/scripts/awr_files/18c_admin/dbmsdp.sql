Rem
Rem $Header: rdbms/admin/dbmsdp.sql /main/69 2017/04/19 10:54:35 bwright Exp $
Rem
...
Rem     START_SW_UPDATE   - Wish to start a sw update for Data Pump/MDAPI
Rem     END_SW_UPDATE     - End a sw update for Data Pump/MDAPI
Rem     START_JOB         - Starts or restarts a job
Rem     STOP_JOB          - Performs an orderly shutdown of a job
Rem     STOP_WORKER       - Kills a worker process that is hung or stuck
...
Rem SQL_SOURCE_FILE: rdbms/admin/dbmsdp.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/dbmsdp.sql
Rem SQL_PHASE: DBMSDP
...
Rem                           definitions
Rem     tmontgom  02/14/17  - Add new dump type KU$_FILE_TYPE_URIDUMP_FILE.
Rem     sdipirro  01/04/17  - Project 68204 - Synchronized sw updates
Rem     rphillip  08/17/16  - Proj 58357: add CONTINUE_LOAD_ON_FORMAT_ERROR
Rem                           to data_options
Rem     sdipirro  08/08/16  - XbranchMerge sdipirro_bug-24309190_12.2.0.1.0
Rem                           from st_rdbms_12.2.0.1.0
Rem     jkaloger  07/26/16  - BUG:22265082 - Add KU$_DFHDR_HADOOP_TRAILERS
...
Rem     sdipirro  11/20/15  - Add get_object_info API for internal users
Rem     rphillip  12/02/15  - Bug 22266179 add verify_stream_format
Rem     sogugupt  06/24/15  - Bug: 19863680 Add Access_method,obj_start_time 
Rem                           and status time in worker list
Rem     rpfau     11/18/14  - Proj 45510: Add DATA_OPTIONS=
Rem                           GROUP_PARTITION_TABLE_DATA to unload data for a
Rem                           partitioned table in one operation.
Rem     rpfau     11/16/14  - More Proj 45510: Add DATA_OPTIONS=
Rem                           TRUST_EXISTING_TABLE_PARTITIONS to load
Rem                           partitions in parallel in a pre-existing table.
Rem     sdipirro  10/23/13  - Add API to kill a worker process
Rem     mwjohnso  07/15/14  - Bug 15979889: Add VALIDATE_TBL_DATA to
Rem                           data_options
Rem     rphillip  03/19/14  - Add KU$_DATAOPT_ENABLE_NET_COMP 
Rem     sdipirro  06/18/13  - Cleanup for long identifiers
Rem     surman    12/29/13  - 13922626: Update SQL metadata
Rem     dgagne    03/06/13  - add test_functionality function
Rem     dgagne    03/12/13  - send rowid with remap data row
Rem     surman    03/27/12  - 13615447: Add SQL patching tags
Rem     jkaloger  03/14/12  - BUG:13696958 - Add REJECT_ROWS_REPCHR data option
Rem     jkaloger  02/20/12  - BUG:13724931 - correct some item codes for
Rem                           dbms_datapump.get_dumpfile_info
Rem     jkaloger  05/24/11  - encryption password work for db consolidation
Rem     jkaloger  05/18/11  - Add ku$_JobStatus1210 and ku$_WorkerStatus1210
Rem     sdavidso  04/12/11  - add import callout KU\$_ICRPP_EARLY
Rem     jkaloger  09/06/09  - Add compression algorithm constants for
Rem                           get_dumpfile_info
Rem     gclaborn  02/14/11  - add 2 new import callout reg. flags
Rem     gclaborn  02/03/11  - Add TRACE_ENTRY
Rem     gclaborn  12/30/10  - Add ku$_JobDesc1210, ku$_Status1210
Rem     jkaloger  12/28/10  - Add message WARNING_DV_NOENCRYPT
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatus1010 FORCE AS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatus1020 FORCE AS OBJECT
        (
                worker_number     NUMBER,       -- Worker process identifier
                process_name      VARCHAR2(30), -- Worker process name
                state             VARCHAR2(30), -- Worker process state
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatus1120 FORCE AS OBJECT
        (
                worker_number     NUMBER,       -- Worker process identifier
                process_name      VARCHAR2(30), -- Worker process name
                state             VARCHAR2(30), -- Worker process state
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatus1210 FORCE AS OBJECT
        (
                worker_number     NUMBER,       -- Worker process identifier
                process_name      VARCHAR2(30), -- Worker process name
                state             VARCHAR2(30), -- Worker process state
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatus1220 FORCE AS OBJECT
        (
                worker_number     NUMBER,       -- Worker process identifier
...
CREATE OR REPLACE TYPE sys.ku$_WorkerStatusList1010 FORCE
AS TABLE OF (sys.ku$_WorkerStatus1010)
NOT PERSISTABLE
/
CREATE OR REPLACE TYPE sys.ku$_WorkerStatusList1020 FORCE
AS TABLE OF (sys.ku$_WorkerStatus1020)
NOT PERSISTABLE
/
CREATE OR REPLACE TYPE sys.ku$_WorkerStatusList1120 FORCE
AS TABLE OF (sys.ku$_WorkerStatus1120)
NOT PERSISTABLE
/
CREATE OR REPLACE TYPE sys.ku$_WorkerStatusList1210 FORCE
AS TABLE OF (sys.ku$_WorkerStatus1210)
NOT PERSISTABLE
/
CREATE OR REPLACE TYPE sys.ku$_WorkerStatusList1220 FORCE
...
CREATE OR REPLACE TYPE sys.ku$_DumpFile1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_DumpFileSet1010 FORCE
...
CREATE OR REPLACE TYPE sys.ku$_LogLine1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_LogEntry1010 FORCE
...
CREATE OR REPLACE TYPE sys.ku$_JobStatus1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_JobStatus1020 FORCE IS OBJECT
        (
                job_name        VARCHAR2(30),           -- Name of the job
                operation       VARCHAR2(30),           -- Current operation
                job_mode        VARCHAR2(30),           -- Current mode
...
CREATE OR REPLACE TYPE sys.ku$_JobStatus1120 FORCE IS OBJECT
        (
                job_name        VARCHAR2(30),           -- Name of the job
                operation       VARCHAR2(30),           -- Current operation
                job_mode        VARCHAR2(30),           -- Current mode
...
CREATE OR REPLACE TYPE sys.ku$_JobStatus1210 FORCE IS OBJECT
        (
                job_name        VARCHAR2(30),           -- Name of the job
                operation       VARCHAR2(30),           -- Current operation
                job_mode        VARCHAR2(30),           -- Current mode
...
CREATE OR REPLACE TYPE sys.ku$_JobStatus1220 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_ParamValue1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_ParamValues1010 FORCE
...
CREATE OR REPLACE TYPE sys.ku$_JobDesc1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_JobDesc1020 FORCE IS OBJECT
        (
                job_name        VARCHAR2(30),           -- The job name
                guid            RAW(16),                -- The job GUID
                operation       VARCHAR2(30),           -- Current operation
...
CREATE OR REPLACE TYPE sys.ku$_JobDesc1210 FORCE IS OBJECT
        (
                job_name        VARCHAR2(30),           -- The job name
                guid            RAW(16),                -- The job GUID
                operation       VARCHAR2(30),           -- Current operation
...
CREATE OR REPLACE TYPE sys.ku$_JobDesc1220 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_Status1010 FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_Status1020 FORCE IS OBJECT
        (
                mask            NUMBER,           -- Status types present
                wip             ku$_LogEntry1010, -- Work in progress
...
CREATE OR REPLACE TYPE sys.ku$_Status1120 FORCE IS OBJECT
        (
                mask            NUMBER,           -- Status types present
                wip             ku$_LogEntry1010, -- Work in progress
                job_description ku$_JobDesc1020,  -- Complete job description
...
CREATE OR REPLACE TYPE sys.ku$_Status1210 FORCE IS OBJECT
        (
                mask            NUMBER,           -- Status types present
                wip             ku$_LogEntry1010, -- Work in progress
...
CREATE OR REPLACE TYPE sys.ku$_Status1220 FORCE IS OBJECT
        (
                mask            NUMBER,           -- Status types present
                wip             ku$_LogEntry1010, -- Work in progress
...
CREATE OR REPLACE TYPE sys.ku$_dumpfile_item FORCE IS OBJECT
        (
...
CREATE OR REPLACE TYPE sys.ku$_dumpfile_info FORCE
...
KU$_FILE_TYPE_URIDUMP_FILE     CONSTANT BINARY_INTEGER := 5;
...
KU$_DUMPFILE_TYPE_URIDISK      CONSTANT BINARY_INTEGER := 4;
...
KU$_STATUS_VERSION_5           CONSTANT NUMBER := 5;
KU$_STATUS_VERSION             CONSTANT NUMBER := KU$_STATUS_VERSION_5;
...
KU$_DFHDR_HADOOP_TRAILERS      CONSTANT NUMBER := 24;
KU$_DFHDR_MAX_ITEM_CODE        CONSTANT NUMBER := 24;
...
KU$_DATAOPT_ENABLE_NET_COMP      CONSTANT NUMBER := 32;
KU$_DATAOPT_GRP_PART_TAB         CONSTANT NUMBER := 64;
KU$_DATAOPT_TRUST_EXIST_TB_PAR   CONSTANT NUMBER := 128;
KU$_DATAOPT_VALIDATE_TBL_DATA    CONSTANT NUMBER := 256;
KU$_DATAOPT_VERIFY_STREAM_FORM   CONSTANT NUMBER := 512;
KU$_DATAOPT_CONT_LD_ON_FMT_ERR   CONSTANT NUMBER := 1024;
...
--
  invalid_argval EXCEPTION; -- OK
    ...
    invalid_argval_num CONSTANT NUMBER := -39001;
...
-- *Action: Correct the bad argument and retry the API.
  ...
  invalid_operation EXCEPTION;
    ...
    invalid_operation_num CONSTANT NUMBER := -39002;
-- "invalid operation"
...
--          redefine the job in a manner that will support the specified API.
  ...
  inconsistent_args EXCEPTION; -- OK
    ...
    inconsistent_args_num CONSTANT NUMBER := -39005;
...
-- *Action: Modify the API call to be consistent with itself.
  ...
  privilege_error EXCEPTION;
    ...
    privilege_error_num CONSTANT NUMBER := -31631;
...
--          contact a database administrator to acquire the needed privileges.
  ...
  invalid_handle EXCEPTION;
    ...
    invalid_handle_num CONSTANT NUMBER := -31623;
...
-- *Action: Make sure handle was returned by DBMS_DATAPUMP.OPEN call.
  ...
  invalid_state EXCEPTION;
    ...
    invalid_state_num CONSTANT NUMBER := -39004;
-- "invalid state"
...
--          state.
  ...
  job_exists EXCEPTION;
    ...
    job_exists_num CONSTANT NUMBER := -31634;
-- "job already exists"
...
--          and re-try the operation (may require a DROP on the Master Table).
  ...
  no_such_job EXCEPTION;
    ...
    no_such_job_num CONSTANT NUMBER := -31626;
-- "job does not exist"
...
--          valid Master Table.
  ...
  internal_error EXCEPTION;
    ...
    internal_error_num CONSTANT NUMBER := -39006;
-- "internal error"
...
-- *Action: Contact Oracle Support.
  ...
  success_with_info EXCEPTION;
    ...
    success_with_info_num CONSTANT NUMBER := -31627;
...
-- *Action: Call DBMS_DATAPUMP.GET_STATUS to retrieve additional information.
  ...
  no_dumpfile_info EXCEPTION;
    ...
    no_dumpfile_info_num CONSTANT NUMBER := -39211;
...
-- *Action: Retry the operation with a valid directory object and filename.
  ...
  warning_dv_noencrypt EXCEPTION;
    PRAGMA EXCEPTION_INIT(warning_dv_noencrypt, -39327);
    warning_dv_noencrypt_num CONSTANT NUMBER := -39327;
...
-- START_SW_UPDATE: Start a Data Pump and MDAPI software update
--
-- PARAMETERS:
--      timeout - Time to wait (in seconds) to acquire the synchronization
--                lock (default of zero returns immediately)
--
-- RETURNS:
--      swlock_handle - Exclusive lock acquired (safe to proceed with update)
--      NULL          - Exclusive lock not acquired
--
-- EXCEPTIONS:
--      NONE
--
...
-- END_SW_UPDATE: End a Data Pump and MDAPI software update
--
-- PARAMETERS:
--      swlock_handle - Lock handle
--
-- RETURNS:
--      NONE
--
...
-- STOP_WORKER: Kills a worker process determined to be stuck or hung
--
-- PARAMETERS:
--      handle       - Identifies the particular job or operation (from
--                     OPEN/ATTACH). This handle is detached on successful
--                     completion of this call.
--      worker_id    - The worker ID as reported by GET_STATUS() in the worker
--                     status.
--      skip_current - Skip the object the worker is currently working on and
--                     appears to be stuck on (default is to retry this object)
--
-- RETURNS:
--      NONE
--
-- EXCEPTIONS:
--      INVALID_HANDLE    - The current session is not attached to this handle
--      INVALID_OPERATION - Specified worker can not be stopped
--      SUCCESS_WITH_INFO - API succeeded but further information available
--                          through the get_status API
--      NO_SUCH_JOB       - The job handle is no longer valid or job no longer
--                          exists
...
-- Retrieve info about an object in the master table
