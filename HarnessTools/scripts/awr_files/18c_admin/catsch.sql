Rem
Rem $Header: rdbms/admin/catsch.sql /main/116 2017/03/08 10:37:33 sankejai Exp $
Rem
Rem catsch.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catsch.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catsch.sql
Rem SQL_PHASE: CATSCH
...
Rem    sankejai    02/22/17 - Bug 25600289: store key in credential
Rem    raeburns    10/31/16 - Bug 23231303: remove OR REPLACE for evolved types
Rem    jlingow     03/18/16 - Create Listeners table for Scheduler
Rem    raeburns    02/29/16 - Bug 22820096: revert ALTER TYPE to default
Rem                           CASCADE
Rem    alestrel    11/03/15 - bug 22119906 : moving alter types from
Rem                           c1201000.sql
Rem    alestrel    09/30/15 - bug 21248604 : Adding index in
Rem                           schedule_notifications for faster exporting
Rem    alestrel    09/25/15 - bug 21647288 : long id in RULE_NAME sys.scheduler
Rem                           _srcq_map
Rem    alestrel    08/13/15 - bug 21539897 : long id in sys.scheduler _job
Rem                           queue_agent
Rem    jlingow     07/02/15 - bug21348198 comment length will now be 4000
...
Rem    pxwong      09/02/14 - proj 36585 
Rem                            scheduler child project for long id project
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    jlingow     10/29/13 - adding notification_owner field to 
Rem                           scheduler$_notification
Rem    paestrad    05/15/12 - Adding connect_credential
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    marccaba    01/12/12 - Bug 13455937: Increasing buffer size of job_style
Rem                           to accomodate for longer job styles.
Rem    jnunezg     09/01/11 - Update job_definition with STORE_OUTPUT
Rem                           attribute.
Rem    evoss       06/20/11 - proj:25225 job resource constraints
Rem    jnunezg     06/17/11 - Restart on recovery/failure support for jobs.
Rem    paestrad    06/15/11 - Longer program and job action sizes
Rem    jnunezg     06/14/11 - Update scheduler$_job_run_details and add new
Rem                           scheduler$_job_out_args
Rem    paestrad    05/24/11 - Allowing null passwords and usernames on
Rem                           credentials
Rem    traney      03/31/11 - 35209: long identifiers dictionary upgrade
Rem    rramkiss    10/04/10 - add index on chain step name and oid for evaluate
...
(
  ...
  comments        varchar2(4000),                        /* program comments */
...
(
  ...
  res_grp_name     varchar2(128),             /* name of assoc resource group */
  ...
  comments         varchar2(4000)                          /* class comments */
)
/
...
(
  ...
  queue_owner     varchar2(128),               /* Owner of event source queue */
  queue_name      varchar2(128),                    /* Source queue for event */
  queue_agent     varchar2(523), /* For secure queues - agent used for 
             subscription to source queue. The size 523 is deduced with
             "unqualified_agent_name"@"schema_name"."queue_name"@"dblink_name"
             where all the names are 128 each, and then add the " and @ 
             together (which is 11)*/
  event_rule      varchar2(261),            /* Rule name associated with this 
  ...
  comments        varchar2(4000),                            /* job comments */
  user_callback    varchar2(386),                   /* User callback routine */
  user_callback_ctx number,  /* Context in which callback should be invoked */
  creator           varchar2(128),           /* original creator of this job */
  ...
  destination       varchar2(261),             /* destination global DB name */
  ...
  credential_name   varchar2(128),                         /* credential name */
  credential_owner  varchar2(128),                        /* credential owner */
  credential_oid    number,                          /* credential object ID */
  owner_udn         varchar2(4000),               /* owner's udn infromation */
  fw_name           varchar2(261),                       /* File watcher name */
  ...
  connect_credential_name   varchar2(128),        /* connect credential name */
  connect_credential_owner  varchar2(128),       /*connect  credential owner */
  connect_credential_oid    number,         /* connect credential object ID */
  pdb_id         number       /* Pluggable database in which to run this job */
)
/
...
(
  ...
  guid              varchar2(32),                       /* GUID of this job */
  char_env          varchar2(4000),               /* Used for Trusted Oracle */
  fw_name           varchar2(261),                      /* File watcher owner */
  fw_oid            number,                /* File watcher ID, if applicable */
  job_status      number,                 /* Job status running/disabled etc */
  next_run_date   timestamp with time zone,/* next date this job will run on */
  last_start_date timestamp with time zone,/* last date on which the job was
  ...
  instance_id       number,          /* instance user requests job to run on */
  dest_oid          number,                               /* destination oid */
  destination       varchar2(261),                    /* Name of destination */
  credential_name   varchar2(128),                         /* credential name */
  credential_owner  varchar2(128),                        /* credential owner */
  credential_oid    number,                          /* credential object ID */
  job_dest_id       number,
...
(
  ...
  name            varchar2(128),                             /* argument name */
...
(
  ...
  res_plan        varchar2(128),                 /* ID of assoc resource plan */
  ...
  scaling_factor  number,            /* The scaling factor to use to determine
                                        ...
                                        CPUs */
  creator              varchar2(128),/* logged-in user who created the window */
  ...
  comments             varchar2(4000)                     /* window comments */
)
/
...
(
  oid            number               NOT NULL,
  rule_name      VARCHAR2(523)        NOT NULL,
...
(
  agt_name       VARCHAR2(128)         NOT NULL
...
(
 ...
 name       varchar2(261),                      /* The name of the object */
 owner      varchar2(128),                    /* The schema of the object */
 ...
 user_name  varchar2(128),                 /* Who performed the operation */
 ...
 flags      number,                                       /* Flags field */
 credential  varchar2(261),                       /* Full credential name */
 destination varchar2(261),                           /* job destination */
...
(
  ...
  session_id      varchar2(128),  /* ID of the session this job ran with */
  ...
  credential  varchar2(261),                       /* Full credential name */
  destination     varchar2(261),           /* destination at which job ran */
  constraint scheduler$_job_run_details_pk PRIMARY KEY (log_id)
    using index (CREATE INDEX sys.i_scheduler_job_run_details
...
(
  ...
  errors          blob,
  output          blob,
  CONSTRAINT scheduler$_job_output_pk UNIQUE (log_id)
            USING INDEX TABLESPACE sysaux
) TABLESPACE sysaux;

alter table sys.scheduler$_job_output
    add constraint scheduler$_job_output_fk FOREIGN KEY (log_id)
    references scheduler$_job_run_details (log_id)
            ON DELETE CASCADE;

create table scheduler$_job_out_args
(
  log_id              number,
  log_date            timestamp with time zone, /* Operation timestamp */
  argument_position   number,       /* Position of out argument */
  argument_name       varchar2(128), /* Name of out argument */
  argument_value      sys.anydata, /* Value generated on job execution */
  argument_type_name  varchar2(128), /* Name of type of agument */
  argument_type_owner varchar2(128), /* Owner of type */
        CONSTRAINT scheduler$_job_out_args_pk
             PRIMARY KEY (log_id, argument_position)
) TABLESPACE sysaux;

CREATE INDEX sys.i_scheduler_job_out_args
  ON sys.scheduler$_job_out_args (log_id)
  TABLESPACE sysaux
/
...
(
  ...
  queue_owner     varchar2(128),               /* Owner of event source queue */
  queue_name      varchar2(128),                /* Name of event source queue */
  queue_agent     varchar2(523),    /* For secure queues - agent used for 
             subscription to source queue. The size 523 is deduced with
             "unqualified_agent_name"@"schema_name"."queue_name"@"dblink_name"
             where all the names are 128 each, and then add the " and @ 
  ...
  comments        varchar2(4000),                        /* schedule comments */
  flags           number,                                   /* schedule type */
  max_count       number,                   /* Maximum number of occurrences */
  fw_name         varchar2(261)                       /* Name of file watcher */
)
/
...
(
  ...
  rule_set        varchar2(128),            /* rule set assoc with this chain */
  rule_set_owner  varchar2(128),                    /* schema of the rule set */
  comments        varchar2(4000),                        /* schedule comments */
...
(
  ...
  var_name        varchar2(128)        NOT NULL,            /* job identifier */
...
(
  ...
  destination     varchar2(261),             /* remote destination for step  */
  flags           number
) TABLESPACE sysaux
/

create or replace type sys.scheduler$_job_step_type FORCE as object (
   state           varchar2(12),
   error_code      number,
...
-- type of upgrade class 2, type with only type dependents 
CREATE OR REPLACE TYPE sys.scheduler$_step_type FORCE IS OBJECT (
...
-- 'END_STEP_ERROR_CODE'
-- type of upgrade class 2, type with only type dependents
CREATE OR REPLACE TYPE sys.scheduler$_chain_link FORCE IS OBJECT (
       first_step_name    VARCHAR2(128),
       first_step_type    VARCHAR2(32),
       second_step_name   VARCHAR2(128),
...
(
  ...
  username        varchar2(128),                    /* operating system user */
  password        varchar2(255),                              /* OS password */
  domain          varchar2(30),                            /* Windows domain */
  comments        varchar2(4000),                  /* user-provided comments */
  flags           number,                          /* flags field (bitfield) */
  key             varchar2(4000)                   /* key for the credential */
)
/
...
CREATE TYPE scheduler_filewatcher_request AS OBJECT
(
  owner                 VARCHAR2(4000),
  name                  VARCHAR2(4000),
...
CREATE TYPE scheduler_filewatcher_result AS OBJECT
(
...
CREATE OR REPLACE TYPE scheduler_filewatcher_history FORCE AS OBJECT
(
  directory_path      VARCHAR2(4000),
...
CREATE INDEX sys.i_scheduler_notification1
  ON scheduler$_notification (notification_owner, owner, job_name, job_subname);
...
/* this speeds up exporting notifications */
CREATE INDEX sys.i_scheduler_notification4
  ON scheduler$_notification (owner, job_name);

/* this speeds up querys that now include notification_owner */
CREATE INDEX sys.i_scheduler_notification5
  ON scheduler$_notification (owner, notification_owner);
...
CREATE TABLE scheduler$_resources
(
  obj#     NUMBER,                                 /* Object ID of resource */
     CONSTRAINT scheduler$_rsrc_pk PRIMARY KEY (obj#),
  resource_count NUMBER, /* Count of the resources that exist on the system */
  query_reserve_callback VARCHAR2(261),     /* Name of callback function to */
                          /*  query or reserve a certain number of resource */
  status   NUMBER, 
  flags    NUMBER,
...
CREATE TABLE  scheduler$_constraints
(
 oid            NUMBER,
 resoid         NUMBER,
 CONSTRAINT scheduler$_const_pk PRIMARY KEY (oid, resoid),
 resource_count NUMBER,
 oid_type       NUMBER
)
/

CREATE TABLE  scheduler$_constraints_stats
(
 oid            NUMBER,
 resoid         NUMBER,
 joboid         NUMBER,
 CONSTRAINT scheduler$_const_stats_pk PRIMARY KEY (oid, resoid, joboid)
            USING INDEX TABLESPACE sysaux,
 last_block     TIMESTAMP WITH TIME ZONE,      /* last time this constraint 
                                                             caused a block */
 total_block_count  NUMBER                       /* total block delay count */
) TABLESPACE sysaux
/

CREATE INDEX sys.i_scheduler_const_stat1
  ON sys.scheduler$_constraints_stats (resoid)
  TABLESPACE sysaux
/

CREATE INDEX sys.i_scheduler_const_stat2
  ON sys.scheduler$_constraints_stats (joboid)
  TABLESPACE sysaux
/

CREATE TYPE sys.scheduler$_dest_list as 
 varray(1024) of VARCHAR2(512)
/

CREATE TYPE scheduler$_remote_arg as object
(
  arg_position       NUMBER,
...
CREATE TYPE scheduler$_remote_arg_list as 
 table of SCHEDULER$_REMOTE_ARG
/

-- type of upgrade class 1, type with table dependents
CREATE TYPE sys.scheduler$_remote_db_job_info as object
(
  operation          NUMBER,
  source             VARCHAR2(512),
...
alter type scheduler$_remote_db_job_info modify attribute
(
  destination_schema  varchar2(128),
...
create type sys.scheduler$_event_info as object
(
...
CREATE OR REPLACE TYPE JOBARG FORCE AS OBJECT
(
  arg_position          NUMBER,
...
CREATE OR REPLACE TYPE JOBARG_ARRAY FORCE AS TABLE OF JOBARG;
/

-- Jobs
-- type of upgrade class 2, but was evolved in upgrade to 12.1
CREATE TYPE JOB_DEFINITION AS OBJECT
(
...
CREATE TYPE JOB_DEFINITION_ARRAY AS TABLE OF JOB_DEFINITION;
/

-- Job attribute

CREATE OR REPLACE TYPE JOBATTR FORCE AS OBJECT
(   
...
-- type of upgrade class 2, type with only type dependents
CREATE OR REPLACE TYPE SCHEDULER$_BATCHERR FORCE AS OBJECT
(
  array_index           NUMBER,
  object_type           VARCHAR2(30),
  object_name           VARCHAR2(394),  /* 3*M_IDEN + 10 */
  attr_name             VARCHAR2(30),
  error_code            NUMBER,
...
CREATE OR REPLACE TYPE SCHEDULER$_BATCHERR_ARRAY FORCE AS 
...
-- type of upgrade class 2, type with only type dependents
CREATE OR REPLACE TYPE JOB FORCE AS OBJECT
(
  -- Job name and classification
  job_name              VARCHAR2(100),  /* 3*M_IDEN + 10 */
  job_class             VARCHAR2(32),
...
-- scheduler$_listeneraddresses table and indexes

CREATE TABLE sys.scheduler$_listeneraddresses
(
  address     varchar2(2000), /* Known Listener protocol address */
  service      varchar2(256), /* Service name available through this listener */
  instance     varchar2(128), /* Reachable instance through this listener */ 
  protocol     number         /* Addresses's protocol type id */
)
/

CREATE INDEX sys.i_scheduler$_listeneraddresses
  ON sys.scheduler$_listeneraddresses (service)
  TABLESPACE sysaux
/

CREATE INDEX sys.i_scheduler$_listeneraddresse2
  ON sys.scheduler$_listeneraddresses (service,instance)
  TABLESPACE sysaux
/

CREATE INDEX sys.i_scheduler$_listeneraddresse3
  ON sys.scheduler$_listeneraddresses (address,service)
  TABLESPACE sysaux
/

CREATE INDEX sys.i_scheduler$_listeneraddresse4
