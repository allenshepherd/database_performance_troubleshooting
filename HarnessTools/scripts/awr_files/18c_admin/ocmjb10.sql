Rem
Rem $Header: emll/admin/scripts/ocmjb10.sql /st_emll_12.1.2.0.1/2 2015/09/09 21:35:13 dkuhn Exp $
Rem
Rem ocmjb10.sql
Rem
...
Rem    dkuhn       09/01/15 - PDB check on create_replace_dir_obj
Rem    dkuhn       08/05/15 - bug 21456791: Add comment
Rem    dkuhn       12/05/14 - bug 20123851: Schedule collection job further in
Rem                           future
Rem    dkuhn       09/16/14 - fixtrans
Rem    dkuhn       09/08/14 - add pdb check
Rem    dkuhn       05/15/14 - Add read only oracle home code
Rem    jsutton     10/09/13 - XbranchMerge jsutton_b-17363183 from
Rem                           st_emll_10.3.8.1
...
BEGIN
  ...
  BEGIN
    dbms_scheduler.disable(stats_job_name, TRUE);
    BEGIN
      dbms_scheduler.stop_job(stats_job_name, TRUE);
    EXCEPTION WHEN others THEN
      IF sqlcode = -27366 THEN NULL; --Suppress job not running error
      ELSE raise;
      END IF;
    END;
    dbms_scheduler.drop_job(stats_job_name, TRUE);
  EXCEPTION WHEN others THEN
    IF sqlcode = -27475 THEN NULL; --Suppress job not existing error
    ELSE raise;
    END IF;
