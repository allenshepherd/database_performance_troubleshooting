...
-----------------------------------------------------------------------------
-- The following structure is used by the function getRestoreRange.
-- startTime and endTime is the starting time and the ending time of restore
-- range respectively. 
-- fromScn and toScn are the lowest and highest scn of the restore range
-- respectively. 
-- isValidRange indicates whether it is a valid restore range 
-----------------------------------------------------------------------------
...
-- Return the db_key of a database if it is set
...
-- Sets highscnval to UB8MAXVAL for databases above 12.2
...
-- rlgscn and lrgtime.
-- If flbrp (flashback CdB to restore point) is TRUE, then allow scn to be
...
-- setSparseness is used to inform the kind of backups to be used
-- for the on-going restore.
--
-- Input parameter:
--   sparseness
--     One of SPARSE, NONSPARSE, or NULL
...
-- setOrsFile is used to limit the translation only to ORS managed files.
--            
-- Input parameters:
--   localOnly
--      TRUE  - only files on local ORS storage
--      FALSE - only files managed by ORS locally or SBT storage
--      NULL  - turn off ORS limitting
--   libKey   - only files on ORS SBT storage with this library key
--      NULL  - turn off ORS limitting
...
-- set First Full Backup file sharing scope attributes for the session
...
--------------------------
-- PrePlugin Translation --
--------------------------
-- translatePrePluginDf translates datafiles which has pre-plugin backups
-- that can be used for restore and recover of PDBs.
...
-----------------------------------------
-- Pluggable Database Name Translation --
-----------------------------------------
...
-- fromSCN > 0 means that find controlfile backup whose checkpoint scn is
-- greater or equal to this scn. This is used to find the right controlfile
-- backup for PDB PITR.
-- pdbid > 1 means that ignore controlfile backup that are in orphan branch
-- pdb incarnation of this pdb.
...
-- Function getRestoreRangeSet finds the set of restore ranges of a database.
...
-- after fromScn. Retrun FALSE if no SCN found else returns TRUE
FUNCTION getNextAvailableSCN(fromScn           IN  NUMBER, 
                             nextAvailableSCN  OUT  NUMBER,
                             isOrs             IN   NUMBER)
  RETURN BOOLEAN;

-- Finds the point where continuity of archived logs breaks
...
-- Added in 12.1
-- Finds out the maximum scn of the redo logs
...
-- Sets the global variable RestoreRangeDevTyp
PROCEDURE setRestoreRangeDevTyp(typ IN VARCHAR2);

-- Resets the global variable RestoreRangeDevTyp
...
-- Added in 12.1
-- Finds out the drop_scn for a datafile 
...
-- set the current controlfile scn in catalog for name translation at
-- standby

FUNCTION translatePdbName(pdbName IN VARCHAR2)
RETURN NUMBER;

-- Converts a pdb name into a pdbid.

FUNCTION translatePdbName(pdbName IN VARCHAR2,
                          pdbGuid OUT VARCHAR2) 
RETURN NUMBER;
-- Converts a pdb name into a pdbguid, pdbid.
...
-- Set the given pdbid in pdbIdList. When first is TRUE, then clear existing
-- list before setting. This list is used for translation of database.

FUNCTION isTranslatedPdbId(pdbId IN NUMBER) RETURN NUMBER;
-- Is this pdbid translated by RMAN?. Returns TRUE# if translated.
-- FALSE# otherwise.
...
-- Sets localOrsSiteKey variable.

PROCEDURE resetLocalOrsSiteKey;
-- Clears localOrsSiteKey variable.

FUNCTION isNoBackupPdb(pdbname IN VARCHAR2) RETURN NUMBER;
-- Return 0 if PDB is not excluded from backup. Otherwise, non-zero.

----------------------------------------
-- Bug 22150623: List Application PDB --
----------------------------------------
...
-- Get app pdbs for given app root

FUNCTION pdbHasDatafile(pdbId IN NUMBER) RETURN NUMBER;
-- Whether the PDB has a datafile: returns TRUE# if yes; otherwise, FALSE#
...
--    unknown    :  > 0 if priority is unknown or ALL. Otherwise, 0.
