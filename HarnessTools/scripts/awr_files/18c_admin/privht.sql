Rem Copyright (c) 1994, 2017, Oracle and/or its affiliates. 
...
Rem
Rem   HISTORY
Rem     cvanes     01/16/17 -  Fix bug 23633556:Remove check for DB 10.2
Rem     cvanes     12/15/11 -  Fix bug 13063704:Handle multibyte surrogate pairs
...
Rem     mmuppago   05/23/06 -  Fix bug# 2136975 - format_cell: fix format string 
Rem     mmuppago   04/27/06 -  Changes to comply with XHTML req. from Portal
...
begin
    ...
    return('<'||ch||
                ...
                IFNOTNULL(cclear,' clear="'||cclear||'"')||
                IFNOTNULL(cnowrap,' nowrap="nowrap"')||
                IFNOTNULL(cattributes,' '||cattributes)||
               '>'||cheader||
               '</'||ch||'>');
end;
...
begin return('<area'||
        ...
        ' coords="'||ccoords||'"'||
                ...
                IFNOTNULL(cnohref,' nohref="nohref"')||
                IFNOTNULL(ctarget,' target="'||ctarget||'"')||
        IFNOTNULL(cattributes,' '||cattributes)||
             ' />'); end;
...
begin 
   return('<input type="checkbox" name="'||cname||'"'||
           ...
           IFNOTNULL(cchecked,' checked="checked"')||
           IFNOTNULL(cattributes,' '||cattributes)||
          ' />');
end;
...
begin return('<option'||
              IFNOTNULL(cselected,' selected="selected"')||
              IFNOTNULL(cattributes,' '||cattributes)||
             '>'||cvalue||'</option>'); end;
...
begin
   ...
   return(tableData(tochar(to_number(columnValue),format_numbers),'right'));
...
begin 
 return('<frame src="'||csrc||'"'||
    ...
    IFNOTNULL(cnoresize, ' noresize="noresize"')||
    IFNOTNULL(cattributes,' '||cattributes)||
    ' />');
end frame;
...
procedure showpage is
   ...
   nl_loc   integer;
begin
   -- On DB version 10.2 and above, dbms_output buffer size can be unlimited
   -- Bugfix 23633556 - assume DB version is 10.2 or above and removed
   -- flawed check.
...
begin
   ...
   if (rows_in < pack_after) then
      ...
      loop
         ...
         if bUseSubstr then
             ...
             htbuf(rows_in) := substr4(cbuf, loc + 1, HTBUF_LEN);
         end if;
         loc := loc + HTBUF_LEN;
      end loop;
      if (loc < len)
      then
         ...
         if bUseSubstr then
             ...
             htbuf(rows_in) := substr4(cbuf, loc + 1);
         end if;
      end if;
      return;
   end if;
   ...
   then
      ...
      if bUseSubstr then
          ...
          tlen := HTBUF_LEN - length4(htcurline);
      end if;
   end if;
   ...
   loop
      ...
      then
         ...
         then
            ...
            if bUseSubstr then
               ...
               htcurline := htcurline || substr4(cbuf, loc + 1);
            end if;
         end if;
         exit;
      end if;
      ...
      if bUseSubstr then
         ...
         htbuf(rows_in) := htcurline || substr4(cbuf, loc + 1, tlen);
      end if;
      htcurline := '';
      loc := loc + tlen;
