rem
rem $Header: rdbms/admin/catprc.sql /main/91 2017/06/26 16:01:18 pjulsaks Exp $
rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catprc.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catprc.sql
Rem SQL_PHASE: CATPRC
...
Rem     rkagarwa   03/07/17  - Bug 25096570: include Table and Synonym
Rem                            dependencies in ALL_DEPENDENCIES
Rem     akruglik   03/08/16  - (22132084): replace COMMON_DATA with
Rem                            SHARING=EXTENDED DATA
Rem     ciyer      12/18/15  - bug 22332196: keep base and _AE views in sync
Rem     sfeinste   10/13/15  - Bug22008202: HCS name changes 
Rem     tfyu       07/22/15  - Bug 17791697
Rem     smesropi   03/04/15  - Bug 20556364: Add hierarchy cube to xxx_errors
Rem     beiyu      12/16/14  - Proj 47091: update xxx_dependencies, xxx_errors
Rem     skayoor    11/30/14  - Proj 58196: Change Select priv to Read Priv
Rem     thbaby     11/10/14  - Proj 47234: remove decode from SHARING column
...
Rem     thbaby     05/08/13  - do not use owner# in INT$DBA_SOURCE
Rem     thbaby     05/08/13  - lrg 8963362: fix ALL_SOURCE by using right alias
Rem     sagrawal   03/05/13  - bug 16105745
...
Rem                            st_rdbms_12.1.0.1
Rem     thbaby     01/14/13  - 15827913: support linked objects in *_SOURCE
Rem     surman     03/27/12  - 13615447: Add SQL patching tags
Rem     traney     07/19/11  - 38543: directory and credential support for
Rem                            libraries
Rem     akruglik   05/18/11  - DB Consolidation: modify *_TRIGGERS views for
Rem                            PDB triggers
Rem     sipatel    10/20/10  - add LOGICAL LOB UPDATE event to *_TRIGGERS 
...
remark
remark    FAMILY "ERRORS" and "ERRORS_AE"
remark    Errors for stored objects - for example types, type bodies,
remark    PL/SQL packages, package bodies, procedures and functions.
remark
remark    The ERRORS_AE family shows errors for objects in all editions
remark
...
decode(o.type#, 4, 'VIEW', 7, 'PROCEDURE', 8, 'FUNCTION', 9, 'PACKAGE',
  ...
  **EDITION-COL**
from **OBJ-VIEW-NAME** o, sys.error$ e
where o.obj# = e.obj#
  and o.type# in (4, 7, 8, 9, 11, 12, 13, 14, 22, 28, 29, 43, 87, 150, 151,
                  152)
  and o.owner# = userenv('SCHEMAID')!';

  base_view_name constant dbms_id := 'user_errors';
...
/
grant read on USER_ERRORS to public with grant option
/

create or replace public synonym USER_ERRORS_AE for USER_ERRORS_AE
/
grant read on USER_ERRORS_AE to public with grant option
/
...
decode(o.type#, 4, 'VIEW', 7, 'PROCEDURE', 8, 'FUNCTION', 9, 'PACKAGE',
  ...
  e.sequence#, e.line, e.position#, e.text,
   decode(e.property, 0,'ERROR', 1, 'WARNING', 'UNDEFINED'), e.error#
  **EDITION-COL**
from **OBJ-VIEW-NAME** o, sys.error$ e, sys.user$ u
where o.obj# = e.obj#
  and o.owner# = u.user#
  and o.type# in (4, 7, 8, 9, 11, 12, 13, 14, 22, 28, 29, 43, 87, 150, 151, 
                  152)
  ...
  base_view_name constant dbms_id := 'all_errors';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
...
/
grant read on ALL_ERRORS to public with grant option
/
create or replace public synonym ALL_ERRORS_AE for ALL_ERRORS_AE
/
grant read on ALL_ERRORS_AE to public with grant option
/
...
where o.obj# = e.obj#
  ...
  base_view_name constant dbms_id := 'dba_errors';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_ERRORS','CDB_ERRORS');
create or replace public synonym CDB_ERRORS for sys.CDB_ERRORS;
grant select on CDB_ERRORS to select_catalog_role;

execute SYS.CDBView.create_cdbview(false,'SYS','DBA_ERRORS_AE','CDB_ERRORS_AE');
create or replace public synonym CDB_ERRORS_AE for sys.CDB_ERRORS_AE;
grant select on CDB_ERRORS_AE to select_catalog_role;

remark
remark    FAMILY "SOURCE" and "SOURCE_AE"
remark    SOURCE for stored objects - for example types, type bodies,
remark    PL/SQL packages, package bodies, procedures and functions.
remark
remark    The SOURCE_AE family shows source for objects in all editions
remark

declare
  view_text constant varchar2(32767) := q'!
create or replace view **VIEW-NAME** SHARING=EXTENDED DATA
(OWNER, NAME, OBJECT_ID, TYPE, TYPE#, LINE, TEXT, **EDITION-COL-NAME**
...
s.line, s.source, **EDITION-COL**
case when bitand(o.flags, 196608)>0 then 1 else 0 end, 
to_number(sys_context('USERENV', 'CON_ID'))
from **OBJ-VIEW-NAME** o, sys.source$ s, sys.user$ u
where o.obj# = s.obj#
  and o.owner# = u.user#
  and ( o.type# in (7, 8, 9, 11, 12, 14, 22) OR
       ( o.type# = 13 AND o.subname is null))
union all
select u.name, o.name, o.obj#, 'JAVA SOURCE', o.type#, s.joxftlno, 
s.joxftsrc, **EDITION-COL**
case when bitand(o.flags, 196608)>0 then 1 else 0 end, 
to_number(sys_context('USERENV', 'CON_ID'))
from **OBJ-VIEW-NAME** o, x$joxscd s, sys.user$ u
where o.obj# = s.joxftobn
  and o.owner# = u.user#
  and o.type# = 28!';

  base_view_name constant dbms_id := 'int$dba_source';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
  ...
  base_ed_col constant varchar2(ora_max_name_len) := '';
  ae_ed_col   constant varchar2(ora_max_name_len*2 + 4) :=
    ' o.defining_edition, ';
...
declare
  view_text constant varchar2(32767) := q'!
create or replace view **VIEW-NAME**
(NAME, TYPE, LINE, TEXT, **EDITION-COL-NAME** ORIGIN_CON_ID)
as
select NAME, TYPE, LINE, TEXT, **EDITION-COL** ORIGIN_CON_ID
from NO_ROOT_SW_FOR_LOCAL(**OBJ-VIEW-NAME**) 
where OWNER = SYS_CONTEXT('USERENV', 'CURRENT_USER')!';

  base_view_name constant dbms_id := 'user_source';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
  ...
  base_obj_view constant varchar2(ora_max_name_len) := 'int$dba_source';
  ae_obj_view   constant varchar2(ora_max_name_len) := base_obj_view || '_ae';
...
comment on column USER_SOURCE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
comment on column USER_SOURCE_AE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
...
/
grant read on USER_SOURCE to public with grant option
/
create or replace public synonym USER_SOURCE_AE for USER_SOURCE_AE
/
grant read on USER_SOURCE_AE to public with grant option
/

declare
  view_text constant varchar2(32767) := q'!
create or replace view **VIEW-NAME**
(OWNER, NAME, TYPE, LINE, TEXT, **EDITION-COL-NAME** ORIGIN_CON_ID)
as
select OWNER, NAME, TYPE, LINE, TEXT, **EDITION-COL** ORIGIN_CON_ID
from **OBJ-VIEW-NAME** ods
where 
  (
  ...
  base_view_name constant dbms_id := 'all_source';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
...
comment on column ALL_SOURCE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
comment on column ALL_SOURCE_AE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
...
/
grant read on ALL_SOURCE to public with grant option
/
create or replace public synonym ALL_SOURCE_AE for ALL_SOURCE_AE
/
grant read on ALL_SOURCE_AE to public with grant option
/
...
from **OBJ-VIEW-NAME**!';
  ...
  base_view_name constant dbms_id := 'dba_source';
  ae_view_name   constant dbms_id := base_view_name || '_ae';
...
comment on column DBA_SOURCE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
comment on column DBA_SOURCE_AE.ORIGIN_CON_ID is
'ID of Container where row originates'
/
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_SOURCE','CDB_SOURCE');
create or replace public synonym CDB_SOURCE for sys.CDB_SOURCE;
grant select on CDB_SOURCE to select_catalog_role;

execute SYS.CDBView.create_cdbview(false,'SYS','DBA_SOURCE_AE','CDB_SOURCE_AE');
create or replace public synonym CDB_SOURCE_AE for sys.CDB_SOURCE_AE;
grant select on CDB_SOURCE_AE to select_catalog_role;
...
/
grant read on USER_TRIGGERS to public with grant option
/
...
/
grant read on ALL_TRIGGERS to public with grant option
/
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_TRIGGERS','CDB_TRIGGERS');
create or replace public synonym CDB_TRIGGERS for sys.CDB_TRIGGERS;
grant select on CDB_TRIGGERS to select_catalog_role;
...
/
grant read on USER_INTERNAL_TRIGGERS to PUBLIC with grant option
/
...
/
grant read on ALL_INTERNAL_TRIGGERS to PUBLIC with grant option
/
...
where t.obj# = o.obj#
      ...
      and bitand(t.trigflag,8) = 8
union
select o.name, u.name, 'SYNC CHANGE TABLE'
from sys.tab$ t, sys.obj$ o, sys.user$ u
where t.obj# = o.obj#
      and u.user# = o.owner#
      and bitand(t.trigflag,16) = 16
union
select o.name, u.name, 'ZONEMAP'
from sys.tab$ t, sys.obj$ o, sys.user$ u
where t.obj# = o.obj#
      and u.user# = o.owner#
      and bitand(t.trigflag,1024) = 1024
union
select o.name, u.name, 'MVIEW STATEMENT REFRESH'
from sys.tab$ t, sys.obj$ o, sys.user$ u
where t.obj# = o.obj#
      and u.user# = o.owner#
      and bitand(t.trigflag,2048) = 2048
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_INTERNAL_TRIGGERS','CDB_INTERNAL_TRIGGERS');
create or replace public synonym CDB_INTERNAL_TRIGGERS for sys.CDB_INTERNAL_TRIGGERS;
grant select on CDB_INTERNAL_TRIGGERS to select_catalog_role;
...
/
grant read on USER_TRIGGER_COLS to public
/
remark
...
/
grant read on ALL_TRIGGER_COLS to public
/
remark
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_TRIGGER_COLS','CDB_TRIGGER_COLS');
create or replace public synonym CDB_TRIGGER_COLS for sys.CDB_TRIGGER_COLS;
grant select on CDB_TRIGGER_COLS to select_catalog_role;
...
/
grant read on USER_DEPENDENCIES to public with grant option
/
...
/
grant read on ALL_DEPENDENCIES to public with grant option
/
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_DEPENDENCIES','CDB_DEPENDENCIES');
create or replace public synonym CDB_DEPENDENCIES for sys.CDB_DEPENDENCIES;
grant select on CDB_DEPENDENCIES to select_catalog_role;
...
/
grant read on PUBLIC_DEPENDENCY to public with grant option
/
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_OBJECT_SIZE','CDB_OBJECT_SIZE');
create or replace public synonym CDB_OBJECT_SIZE for sys.CDB_OBJECT_SIZE;
grant select on CDB_OBJECT_SIZE to select_catalog_role;
...
/
grant read on USER_OBJECT_SIZE to public with grant option
/
...
execute SYS.CDBView.create_cdbview(false,'SYS','DBA_TRIGGER_ORDERING','CDB_TRIGGER_ORDERING');
create or replace public synonym CDB_TRIGGER_ORDERING for sys.CDB_TRIGGER_ORDERING;
grant select on CDB_TRIGGER_ORDERING to select_catalog_role;
...
/
grant read on ALL_TRIGGER_ORDERING to public with grant option
/
...
/
grant read on USER_TRIGGER_ORDERING to public with grant option
/
comment on table USER_TRIGGERS is
