Rem
Rem $Header: rdbms/admin/catstrt.sql /main/16 2015/08/19 11:54:50 raeburns Exp $
Rem
Rem catstrt.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catstrt.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catstrt.sql
Rem SQL_PHASE: CATSTRT
...
Rem    raeburns    05/31/15 - Use FORCE for types with only type dependent
Rem    skayoor     11/30/14 - Proj 58196: Change Select priv to Read Priv
Rem    sgarduno    05/26/14 - Bug 18030980: Long Identifiers support, use 128
Rem                           instead of M_IDEN.
Rem    surman      12/29/13 - 13922626: Update SQL metadata
Rem    tianli      12/09/13 - fix 17742001: don't register process_drop_user
Rem    talliu      06/28/13 - Add CDB view for DBA view
Rem    surman      03/27/12 - 13615447: Add SQL patching tags
Rem    lzheng      11/01/11 - Add DBA_XSTREAM_TRANSFORMATIONS
Rem    yurxu       04/21/11 - Add ALL_XSTREAM_ADMINISTRATOR
...
(
  rule_owner           varchar2(128) not null,                  /* rule owner */
  rule_name            varchar2(128) not null,                   /* rule name */
  declarative_type     number,                  /* The type of the transform */
  from_schema_name     varchar2(128),                      /* old schema name */
  to_schema_name       varchar2(128),                      /* new schema name */
  from_table_name      varchar2(128),                       /* old table name */
  ...
  column_function      varchar2(128),                 /* column function name */
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_TRANSFORM_FUNCTION','CDB_STREAMS_TRANSFORM_FUNCTION');
grant select on SYS.CDB_STREAMS_TRANSFORM_FUNCTION to select_catalog_role
/
create or replace public synonym CDB_STREAMS_TRANSFORM_FUNCTION for SYS.CDB_STREAMS_TRANSFORM_FUNCTION
/
...
/
grant read on ALL_STREAMS_TRANSFORM_FUNCTION to public with grant option
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_ADMINISTRATOR','CDB_STREAMS_ADMINISTRATOR');
grant select on SYS.CDB_STREAMS_ADMINISTRATOR to select_catalog_role
/
create or replace public synonym CDB_STREAMS_ADMINISTRATOR for SYS.CDB_STREAMS_ADMINISTRATOR
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_XSTREAM_ADMINISTRATOR','CDB_XSTREAM_ADMINISTRATOR');
grant select on SYS.CDB_XSTREAM_ADMINISTRATOR to select_catalog_role
/
create or replace public synonym CDB_XSTREAM_ADMINISTRATOR for SYS.CDB_XSTREAM_ADMINISTRATOR
/
...
create or replace view "_DBA_SXGG_TRANSFORMATIONS"
...
as 
select st.*
from "_DBA_SXGG_TRANSFORMATIONS" st 
where (st.rule_owner, st.rule_name) NOT IN 
      (select rule_owner, rule_name from DBA_XSTREAM_RULES
       union
       select rule_owner, rule_name from DBA_GOLDENGATE_RULES
      )
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_TRANSFORMATIONS','CDB_STREAMS_TRANSFORMATIONS');
grant select on SYS.CDB_STREAMS_TRANSFORMATIONS to select_catalog_role
/
create or replace public synonym CDB_STREAMS_TRANSFORMATIONS for SYS.CDB_STREAMS_TRANSFORMATIONS
/

----------------------------------------------------------------------------
-- DBA_XSTREAM_TRANSFORMATIONS
----------------------------------------------------------------------------
create or replace view DBA_XSTREAM_TRANSFORMATIONS
as
select st.* 
from  "_DBA_SXGG_TRANSFORMATIONS" st
where (st.rule_owner, st.rule_name) in
      (select rule_owner, rule_name from DBA_XSTREAM_RULES)
/

comment on table DBA_XSTREAM_TRANSFORMATIONS is
'Transformations defined on rules'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.RULE_OWNER is
'Owner of the rule which has an associated transformation'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.RULE_NAME is
'Name of the rule which has an associated transformation'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.TRANSFORM_TYPE is
'The type of transformation'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.FROM_SCHEMA_NAME is
'The schema to be renamed'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.TO_SCHEMA_NAME is
'The new schema name'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.FROM_TABLE_NAME is
'The table to be renamed'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.TO_TABLE_NAME is
'The new table name'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.SCHEMA_NAME is
'The schema of the column to be modified'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.TABLE_NAME is
'The table of the column to be modified'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.FROM_COLUMN_NAME is
'The column to rename'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.TO_COLUMN_NAME is
'The new column name'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.COLUMN_NAME is
'The column to add or delete'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.COLUMN_VALUE is
'The value of the column to add'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.COLUMN_TYPE is
'The type of the new column'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.COLUMN_FUNCTION is
'The name of the default function used to add a column'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.USER_FUNCTION_NAME is
'The name of the user-defined transformation function to run '
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.SUBSETTING_OPERATION is
'DML operation for row subsetting'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.DML_CONDITION is
'Row subsetting condition'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.DECLARATIVE_TYPE is
'The type of declarative transformation to run'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.PRECEDENCE is
'Execution order relative to other declarative transformations on the same step_number'
/
comment on column DBA_XSTREAM_TRANSFORMATIONS.STEP_NUMBER is
'The order that this transformation should be executed'
/

create or replace public synonym DBA_XSTREAM_TRANSFORMATIONS
  for DBA_XSTREAM_TRANSFORMATIONS
/
grant select on DBA_XSTREAM_TRANSFORMATIONS to select_catalog_role
/
  

execute CDBView.create_cdbview(false,'SYS','DBA_XSTREAM_TRANSFORMATIONS','CDB_XSTREAM_TRANSFORMATIONS');
grant select on SYS.CDB_XSTREAM_TRANSFORMATIONS to select_catalog_role
/
create or replace public synonym CDB_XSTREAM_TRANSFORMATIONS for SYS.CDB_XSTREAM_TRANSFORMATIONS
/
...
----------------------------------------------------------------------------
create or replace view ALL_XSTREAM_TRANSFORMATIONS as
select st.*
from  DBA_XSTREAM_TRANSFORMATIONS st, ALL_APPLY aa, ALL_CAPTURE ca
  where (aa.apply_user = st.rule_owner) or (ca.capture_user = st.rule_owner)
/

comment on table ALL_XSTREAM_TRANSFORMATIONS is
'Transformations defined on rules for the user'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.RULE_OWNER is
'Owner of the rule which has an associated transformation'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.RULE_NAME is
'Name of the rule which has an associated transformation'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.TRANSFORM_TYPE is
'The type of transformation'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.FROM_SCHEMA_NAME is
'The schema to be renamed'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.TO_SCHEMA_NAME is
'The new schema name'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.FROM_TABLE_NAME is
'The table to be renamed'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.TO_TABLE_NAME is
'The new table name'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.SCHEMA_NAME is
'The schema of the column to be modified'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.TABLE_NAME is
'The table of the column to be modified'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.FROM_COLUMN_NAME is
'The column to rename'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.TO_COLUMN_NAME is
'The new column name'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.COLUMN_NAME is
'The column to add or delete'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.COLUMN_VALUE is
'The value of the column to add'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.COLUMN_TYPE is
'The type of the new column'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.COLUMN_FUNCTION is
'The name of the default function used to add a column'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.USER_FUNCTION_NAME is
'The name of the user-defined transformation function to run '
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.SUBSETTING_OPERATION is
'DML operation for row subsetting'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.DML_CONDITION is
'Row subsetting condition'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.DECLARATIVE_TYPE is
'The type of declarative transformation to run'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.PRECEDENCE is
'Execution order relative to other declarative transformations on the same step_number'
/
comment on column ALL_XSTREAM_TRANSFORMATIONS.STEP_NUMBER is
'The order that this transformation should be executed'
/

create or replace public synonym ALL_XSTREAM_TRANSFORMATIONS
  for ALL_XSTREAM_TRANSFORMATIONS
/
grant select on ALL_XSTREAM_TRANSFORMATIONS to select_catalog_role
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_RENAME_SCHEMA','CDB_STREAMS_RENAME_SCHEMA');
grant select on SYS.CDB_STREAMS_RENAME_SCHEMA to select_catalog_role
/
create or replace public synonym CDB_STREAMS_RENAME_SCHEMA for SYS.CDB_STREAMS_RENAME_SCHEMA
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_RENAME_TABLE','CDB_STREAMS_RENAME_TABLE');
grant select on SYS.CDB_STREAMS_RENAME_TABLE to select_catalog_role
/
create or replace public synonym CDB_STREAMS_RENAME_TABLE for SYS.CDB_STREAMS_RENAME_TABLE
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_DELETE_COLUMN','CDB_STREAMS_DELETE_COLUMN');
grant select on SYS.CDB_STREAMS_DELETE_COLUMN to select_catalog_role
/
create or replace public synonym CDB_STREAMS_DELETE_COLUMN for SYS.CDB_STREAMS_DELETE_COLUMN
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_KEEP_COLUMNS','CDB_STREAMS_KEEP_COLUMNS');
grant select on SYS.CDB_STREAMS_KEEP_COLUMNS to select_catalog_role
/
create or replace public synonym CDB_STREAMS_KEEP_COLUMNS for SYS.CDB_STREAMS_KEEP_COLUMNS
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_RENAME_COLUMN','CDB_STREAMS_RENAME_COLUMN');
grant select on SYS.CDB_STREAMS_RENAME_COLUMN to select_catalog_role
/
create or replace public synonym CDB_STREAMS_RENAME_COLUMN for SYS.CDB_STREAMS_RENAME_COLUMN
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_STREAMS_ADD_COLUMN','CDB_STREAMS_ADD_COLUMN');
grant select on SYS.CDB_STREAMS_ADD_COLUMN to select_catalog_role
/
create or replace public synonym CDB_STREAMS_ADD_COLUMN for SYS.CDB_STREAMS_ADD_COLUMN
/
...
/* bug 17742001: we no longer register process_drop_user_cascade to $duc since */
/*               it is called within kzudrp, so remove the insert */
commit;
 ...
 * The only dependent of this type is the Array defined below, so any changes
 * to the type may need to be propagated to queue tables programatically */
CREATE OR REPLACE TYPE sys.streams$nv_node FORCE
