Rem
Rem $Header: rdbms/admin/dbmscap.sql /main/32 2017/03/10 09:04:59 abrown Exp $
Rem
Rem dbmscap.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/dbmscap.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/dbmscap.sql
Rem SQL_PHASE: DBMSCAP
...
Rem    abrown      02/17/17 - bug 25513070 : alter extract start_scn to less
Rem                           than original start_scn
Rem    fergutie    08/01/15 - Bug 24325339, 24324702, 24325184
Rem    fergutie    09/22/14 - Add warning during create_capture if
Rem                           _SOURCE_DB_TIMEZONE parameter is required
...
Rem    tianli      12/08/11 - rename create_capture parameter purpose
Rem    tianli      12/01/11 - fix prepare calls
...
CREATE OR REPLACE PACKAGE dbms_capture_adm AUTHID CURRENT_USER AS
  ...
  create_capture_proc EXCEPTION;
    ...
    create_capture_proc_num CONSTANT NUMBER := -26678;

  inv_sync_capture_proc EXCEPTION;
    PRAGMA exception_init(inv_sync_capture_proc, -25338);
    inv_sync_capture_proc_num  CONSTANT  NUMBER := -25338;

  exp_sync_capture      EXCEPTION;
    PRAGMA exception_init(exp_sync_capture, -25339);
    exp_sync_capture_num  CONSTANT NUMBER := -25339;

  source_timezone_required EXCEPTION;
    PRAGMA exception_init(source_timezone_required, -26956);
    source_timezone_required_num CONSTANT NUMBER := -26956;
