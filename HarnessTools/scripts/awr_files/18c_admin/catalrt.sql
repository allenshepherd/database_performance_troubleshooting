Rem
Rem $Header: rdbms/admin/catalrt.sql /main/28 2017/06/26 16:01:18 pjulsaks Exp $
Rem
Rem catalrt.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catalrt.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catalrt.sql
Rem SQL_PHASE: CATALRT
...
Rem    ilistvin    03/03/11 - CDB changes
...
CREATE TABLE wri$_alert_history(
      ...
      pdb_name                 VARCHAR2(128),
      con_id                   NUMBER,
      CONSTRAINT wri$_alert_history_pk PRIMARY KEY (
           sequence_id,
...
-- Note: we can not have a PRIMARY KEY constraint 
--       on this table because we want to be able
--       to set t_object_name to NULL.
--
...
execute CDBView.create_cdbview(false,'SYS','DBA_ALERT_ARGUMENTS','CDB_ALERT_ARGUMENTS');
grant select on SYS.CDB_alert_arguments to select_catalog_role
/
create or replace public synonym CDB_alert_arguments for SYS.CDB_alert_arguments
/
