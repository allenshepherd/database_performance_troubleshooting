Rem
Rem $Header: rdbms/admin/catxdbv.sql /main/64 2017/04/26 07:42:12 luisgarc Exp $
Rem
Rem catxdbv.sql
Rem
...
Rem SQL_SOURCE_FILE: rdbms/admin/catxdbv.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catxdbv.sql
Rem SQL_PHASE: CATXDBV
...
Rem    luisgarc    04/20/17 - Bug 25871651: Change substr to substrb
Rem    dmelinge    03/23/16 - AddSchemaLocMapping for XDB, bug 22916987
Rem    jcanovi     03/02/15 - use tab$.property instead of tab$.property2
Rem    qyu         01/15/15 - #20355040: fix sys priv check
Rem    skayoor     11/30/14 - Proj 58196: Change Select priv to Read Priv
Rem    jcanovi     11/05/14 - lrg13752073: fix dba_xml_tables
Rem    jcanovi     09/26/14 - 19706679: Added token set column to catalog views
Rem    raeburns    07/07/14 - remove script invocations for reload restructure
Rem    surman      01/23/14 - 13922626: Update SQL metadata
Rem    talliu      01/14/14 - 18057113: create public synonym for 
Rem                           CDB_XML_SCHEMA_INCLUDES
Rem    raeburns    12/17/13 - add rclist view
Rem    sasounda    11/19/13 - 17746252: handle KZSRAT when creating all_* views
Rem    qyu         08/08/13 - xml_tab_cols: check tc property for rowinfo col
Rem    qyu         07/09/13 - #11658581: fix user_xml_tab_cols
Rem    talliu      06/28/13 - Add CDB view for DBA view
Rem    talliu      06/28/13 - Add CDB view for DBA view
Rem    qyu         03/18/13 - Common start and end scripts
Rem    atabar      02/16/12 - 8368788: wrap xml_schema_name_present
Rem    atabar      02/08/12 - 13031694 fix _xml_tables/views/view_cols/tab_cols
Rem    bhammers    11/12/11 - remove catxdbvfexp.sql
Rem    bhammers    05/18/11 - add catvxutil.sql
Rem    spetride    05/10/11 - calling catxdbvfexp.sql
Rem    attran      08/10/10 - add OR to type in DBA/ALL_XML_INDEXES
...
-- The following view consists of 4 main select blocks and then we union all
-- results. The need of different selects arises from the different kinds of
-- XML table/columns depending on those different results would be shown.
-- For example, for schema based tables we show different information about
-- the schema, also, when we have a binary XML table supporting Granular token
-- sets we show the token set information. Only some table have this info
-- available, so we seperate in different selects each type of tables 
-- regarding the information to be displayed. So the first select is for
-- all XML tables non-schema based and not supporting granular token sets.
-- The second select stands for schema based not supporting granular token sets
-- The third lists all non-schema based tables supporting granular token sets.
-- Finally, we select all schema based tables supporting granular token sets.
-- The same approach is used for ALL_XML_TABLES, USER_XML_TABLES,
-- DBA_XML_TABCOLS, ALL_XML_TABCOLS, and USER_XML_TABCOLS views.
create or replace force view DBA_XML_TABLES
 (OWNER, TABLE_NAME, XMLSCHEMA, SCHEMA_OWNER, ELEMENT_NAME, STORAGE_TYPE,
  ANYSCHEMA, NONSCHEMA, TOKENSETS)
...
where o.owner# = u.user#
  ...
  and ts.obj#=o.obj#
/
show error view DBA_XML_TABLES;
...
-- XDB$RCLIST view
create or replace view xdb.xdb$rclist_v as (select rclist from xdb.xdb$root_info);

-- This is needed for users to be able to query the repository's rclist
grant read on xdb.xdb$rclist_v to public;

execute CDBView.create_cdbview(false,'SYS','DBA_XML_TABLES','CDB_XML_TABLES');
grant select on SYS.CDB_XML_TABLES to select_catalog_role
/
create or replace public synonym CDB_XML_TABLES for SYS.CDB_XML_TABLES
/
...
where o.owner# = u.user#
  ...
  and bitand(t.property, power(2,64)) = power(2,64)
/
show error view ALL_XML_TABLES;

grant read on all_xml_tables to public;
...
where o.owner# = userenv('SCHEMAID')
  ...
  and ts.obj#=o.obj#
/
show error view USER_XML_TABLES;

grant read on user_xml_tables to public;
...
create or replace force view DBA_XML_TAB_COLS
 (OWNER, TABLE_NAME, COLUMN_NAME, XMLSCHEMA, SCHEMA_OWNER,
  ELEMENT_NAME, STORAGE_TYPE, ANYSCHEMA, NONSCHEMA, TOKENSETS)
...
where o.owner# = u.user#
  ...
  and bitand(opq.flags,69) = 68
/
show error view DBA_XML_TAB_COLS;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_TAB_COLS','CDB_XML_TAB_COLS');
grant select on SYS.CDB_XML_TAB_COLS to select_catalog_role
/
create or replace public synonym CDB_XML_TAB_COLS for SYS.CDB_XML_TAB_COLS
/
...
/
show error view ALL_XML_TAB_COLS;

grant read on all_xml_tab_cols to public;
...
decode(xel.xmldata.property.name, null,
 ...
 where o.owner# = userenv('SCHEMAID')
  ...
  and opq.elemnum =  xel.xmldata.property.prop_number
/
show error view USER_XML_TAB_COLS;

grant read on user_xml_tab_cols to public;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_VIEWS','CDB_XML_VIEWS');
grant select on SYS.CDB_XML_VIEWS to select_catalog_role
/
create or replace public synonym CDB_XML_VIEWS for SYS.CDB_XML_VIEWS
/
...
/
grant read on all_xml_views to public;
...
where o.owner# = userenv('SCHEMAID')
  ...
  and opq.elemnum =  xel.xmldata.property.prop_number
/
grant read on user_xml_views to public;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_VIEW_COLS','CDB_XML_VIEW_COLS');
grant select on SYS.CDB_XML_VIEW_COLS to select_catalog_role
/
create or replace public synonym CDB_XML_VIEW_COLS for SYS.CDB_XML_VIEW_COLS
/
...
/
grant read on all_xml_view_cols to public;
...
where o.owner# = userenv('SCHEMAID')
  ...
  and opq.elemnum =  xel.xmldata.property.prop_number
/
grant read on user_xml_view_cols to public;
...
--Bug 22916987, select needed for AddSchemaLocMapping for XDB
grant select on dba_xml_schemas to xdb;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_SCHEMAS','CDB_XML_SCHEMAS');
grant select on SYS.CDB_XML_SCHEMAS to select_catalog_role
/
create or replace public synonym CDB_XML_SCHEMAS for SYS.CDB_XML_SCHEMAS
/
...
/
grant read on all_xml_schemas to public with grant option;
 ...
 as select u.name, s.xmldata.schema_url,
    ...
    and s.xmldata.schema_url
       not in (select s2.xmldata.schema_url
               ...
               and   u2.name  = s.xmldata.schema_owner)
/
grant read on all_xml_schemas2 to public with grant option;
...
/
grant read on user_xml_schemas to public with grant option;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_INDEXES','CDB_XML_INDEXES');
grant select on SYS.CDB_XML_INDEXES to select_catalog_role
/
create or replace public synonym CDB_XML_INDEXES for SYS.CDB_XML_INDEXES
/
...
/
show errors;
grant read on all_xml_indexes to public;
...
/
show errors;
grant read on user_xml_indexes to public;
...
grant read on USER_XML_COLUMN_NAMES to public;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_SCHEMA_IMPORTS','CDB_XML_SCHEMA_IMPORTS');
grant select on SYS.CDB_XML_SCHEMA_IMPORTS to select_catalog_role
/
create or replace public synonym CDB_XML_SCHEMA_IMPORTS for SYS.CDB_XML_SCHEMA_IMPORTS
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_SCHEMA_INCLUDES','CDB_XML_SCHEMA_INCLUDES');
grant select on SYS.CDB_XML_SCHEMA_INCLUDES to select_catalog_role
/
create or replace public synonym CDB_XML_SCHEMA_INCLUDES for SYS.CDB_XML_SCHEMA_INCLUDES;
...
execute CDBView.create_cdbview(false,'SYS','DBA_XML_SCHEMA_DEPENDENCY','CDB_XML_SCHEMA_DEPENDENCY');
grant select on SYS.CDB_XML_SCHEMA_DEPENDENCY to select_catalog_role
/
create or replace public synonym CDB_XML_SCHEMA_DEPENDENCY for SYS.CDB_XML_SCHEMA_DEPENDENCY
/
...
execute CDBView.create_cdbview(false,'SYS','DBA_XMLSCHEMA_LEVEL_VIEW_DUP','CDB_XMLSCHEMA_LEVEL_VIEW_DUP');
grant select on SYS.CDB_XMLSCHEMA_LEVEL_VIEW_DUP to select_catalog_role
/
create or replace public synonym CDB_XMLSCHEMA_LEVEL_VIEW_DUP for SYS.CDB_XMLSCHEMA_LEVEL_VIEW_DUP
/
...
grant read on DBA_XMLSCHEMA_LEVEL_VIEW to public
/
create or replace public synonym DBA_XMLSCHEMA_LEVEL_VIEW for DBA_XMLSCHEMA_LEVEL_VIEW;

execute CDBView.create_cdbview(false,'SYS','DBA_XMLSCHEMA_LEVEL_VIEW','CDB_XMLSCHEMA_LEVEL_VIEW');

grant select on SYS.CDB_XMLSCHEMA_LEVEL_VIEW to select_catalog_role
/
create or replace public synonym CDB_XMLSCHEMA_LEVEL_VIEW for SYS.CDB_XMLSCHEMA_LEVEL_VIEW
/
