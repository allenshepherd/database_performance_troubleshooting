...
Rem SQL_SOURCE_FILE: rdbms/admin/catadrvw.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/catadrvw.sql
Rem SQL_PHASE: CATADRVW
...
Rem    svaziran    04/28/16 - 23192127: add pdb_space_mgmt
Rem    svaziran    12/24/15 - 22468424: add pdb_problem
Rem    dmmorton    11/17/14 - proj 47362: purge relation, fixing adrvw scripts
Rem    tseibold    02/03/14 - 17551365: is_adr_disabled
...
create or replace view 
	v_$diag_LOG_EXT as select * from x$diag_LOG_EXT;
create or replace public synonym 
	v$diag_LOG_EXT for v_$diag_LOG_EXT;
grant select on v_$diag_LOG_EXT to dba;
...
create or replace view 
	v_$diag_DFW_PATCH_CAPTURE as select * from x$diag_DFW_PATCH_CAPTURE;
create or replace public synonym 
	v$diag_DFW_PATCH_CAPTURE for v_$diag_DFW_PATCH_CAPTURE;
grant select on v_$diag_DFW_PATCH_CAPTURE to dba;

create or replace view 
	v_$diag_DFW_PATCH_ITEM as select * from x$diag_DFW_PATCH_ITEM;
create or replace public synonym 
	v$diag_DFW_PATCH_ITEM for v_$diag_DFW_PATCH_ITEM;
grant select on v_$diag_DFW_PATCH_ITEM to dba;

create or replace view 
	v_$diag_DFW_PURGE as select * from x$diag_DFW_PURGE;
create or replace public synonym 
	v$diag_DFW_PURGE for v_$diag_DFW_PURGE;
grant select on v_$diag_DFW_PURGE to dba;

create or replace view 
	v_$diag_DFW_PURGE_ITEM as select * from x$diag_DFW_PURGE_ITEM;
create or replace public synonym 
	v$diag_DFW_PURGE_ITEM for v_$diag_DFW_PURGE_ITEM;
grant select on v_$diag_DFW_PURGE_ITEM to dba;

create or replace view 
	v_$diag_ADR_CONTROL_AUX as select * from x$diag_ADR_CONTROL_AUX;
create or replace public synonym 
	v$diag_ADR_CONTROL_AUX for v_$diag_ADR_CONTROL_AUX;
grant select on v_$diag_ADR_CONTROL_AUX to dba;

create or replace view 
	v_$diag_PDB_PROBLEM as select * from x$diag_PDB_PROBLEM;
create or replace public synonym 
	v$diag_PDB_PROBLEM for v_$diag_PDB_PROBLEM;
grant select on v_$diag_PDB_PROBLEM to dba;

create or replace view 
	v_$diag_PDB_SPACE_MGMT as select * from x$diag_PDB_SPACE_MGMT;
create or replace public synonym 
	v$diag_PDB_SPACE_MGMT for v_$diag_PDB_SPACE_MGMT;
grant select on v_$diag_PDB_SPACE_MGMT to dba;

DECLARE
   is_adr_disabled number := 0;


BEGIN
select count(*) into is_adr_disabled
from V$PARAMETER
where name='_diag_adr_enabled' and value='FALSE';


IF (is_adr_disabled != 0) THEN
  RAISE_APPLICATION_ERROR(-20000, 'ADR is not enabled -- set _diag_adr_enabled to TRUE and execute catadrvw again');
END IF;
END;
/
...
create or replace view 
	v_$diag_VPDB_PROBLEM as select * from x$diag_VPDB_PROBLEM;
create or replace public synonym 
	v$diag_VPDB_PROBLEM for v_$diag_VPDB_PROBLEM;
grant select on v_$diag_VPDB_PROBLEM to dba;
