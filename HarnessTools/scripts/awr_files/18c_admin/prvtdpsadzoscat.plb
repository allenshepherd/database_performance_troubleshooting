...
AS SELECT
  ...
  CAST ((AP.OBJECT_ID * 1000) + AP.ORIGIN_CON_ID AS NUMBER(10)) ROUTINEID,
  ...
  FROM ALL_PROCEDURES AP LEFT OUTER JOIN ALL_ARGUMENTS AA
    ON  AP.OBJECT_ID      = AA.OBJECT_ID -- function return parameter
       AND AP.ORIGIN_CON_ID = AA.ORIGIN_CON_ID
       AND AP.OBJECT_TYPE = 'FUNCTION'
       AND AA.PACKAGE_NAME  IS NULL
       AND AA.DATA_LEVEL  = 0
       AND AA.POSITION    = 0
    LEFT OUTER JOIN (SELECT OBJECT_ID, ORIGIN_CON_ID, MAX(POSITION) MAXPOS
                       FROM ALL_ARGUMENTS
                       WHERE PACKAGE_NAME IS NULL
                          AND DATA_LEVEL = 0
                       GROUP BY OBJECT_ID, ORIGIN_CON_ID) AA2
    ON  AP.OBJECT_ID      = AA2.OBJECT_ID
       AND AP.ORIGIN_CON_ID = AA2.ORIGIN_CON_ID
...
AS SELECT
  ...
  CAST ((AA.OBJECT_ID * 1000) + AA.ORIGIN_CON_ID AS NUMBER(10)) ROUTINEID,
  CASE IN_OUT
    WHEN 'IN'     THEN 'P'
    WHEN 'OUT'
      THEN CASE
