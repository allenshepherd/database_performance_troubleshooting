Rem 
Rem $Header: plsql/admin/pidian.sql /main/71 2017/05/22 13:40:19 wxli Exp $ 
Rem 
...
Rem SQL_SOURCE_FILE: plsql/admin/pidian.sql
Rem SQL_SHIPPED_FILE: rdbms/admin/pidian.sql
Rem SQL_PHASE: PIDIAN
...
Rem     wxli       05/17/17 - lrg 20298171: use create or replace diana
Rem     rdecker    10/16/15 - 18734711: add a_EXT_FLAGS
Rem     jolkin     02/07/15 - extend conversion operators
Rem     sagrawal   04/15/14 - Polymorphic Table Functions
Rem     surman     01/09/14 - 13922626: Update SQL metadata
Rem     surman     03/27/12 - 13615447: Add SQL patching tags
Rem     rdecker    08/30/11 - cleanup OID nodes
Rem     traney     08/07/11 - 38543: directory and credential support for libraries
Rem     astocks    06/15/11 - proj 33067 - whitelist
Rem     rdecker    02/18/11 - add as_toid
Rem     rdecker    02/10/11 - remove q_table for now
Rem     rdecker    01/10/11 - Regen Diana
Rem     rdecker    01/01/11 - add TOID/IDX to Q_TABLE for subtypes
Rem     rdecker    12/31/10 - Add S_TYPIDX
Rem     rdecker    12/15/10 - TOIDs
Rem     rdecker    12/15/10 - TOIDs
Rem     jmuller    11/30/10 - Fix bug 10315709: regenerate
...
create or replace package sys.diana is
  ...
  D_CHARSET_SPEC constant pidl.ptnty := 252;
  ...
  D_ALTERN_EXP constant pidl.ptnty := 290;
  ...
  D_IMPL_BODY constant pidl.ptnty := 295;
  ...
  D_SUBPROG_PROP constant pidl.ptnty := 299;
  ...
  function  a_CRED(node pidl.ptnod) return pidl.ptnod;

  function  a_DIR(node pidl.ptnod) return pidl.ptnod;

  function  as_OIDTYPS(node pidl.ptnod) return pidl.ptseqnd;

  function  as_WHTLST(node pidl.ptnod) return pidl.ptnod;

  function  d_WHTLST_KIND(node pidl.ptnod) return pidl.ptnod;

  function  d_WHTLST_NAME(node pidl.ptnod) return pidl.ptnod;

  function  d_WHTLST_SCHEMA(node pidl.ptnod) return pidl.ptnod;

  function  s_ALS(node pidl.ptnod) return varchar2;

  function  s_TOID(node pidl.ptnod) return varchar2;

  function  a_BITFLAGS2(node pidl.ptnod) return pidl.ub4;

  function  a_EXT_FLAGS(node pidl.ptnod) return pidl.ub4;

  function  a_ON_ERR(node pidl.ptnod) return pidl.ptnod;
...
-- This is a generated file. See genidl.pls.
create or replace package body sys.diana is
